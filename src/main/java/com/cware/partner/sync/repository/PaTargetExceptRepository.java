...
public interface PaTargetExceptRepository extends JpaRepository<PaTargetExcept, String> {
	...
	@Query(value = "select count(brand.brand_code) "
			+ "  from tpaqeentargetbrand brand"
			+ "     , tpaqeentargetentp entp "
			+ " where brand.brand_code = entp.brand_code(+) "
			+ "   and brand.brand_code = :brandCode "
			+ "   and brand.use_yn = '1' "
			+ "   and entp.use_yn(+) = '1' "
			+ "   and ( brand.all_entp_yn = '1' "
			+ "    or  entp.entp_code = :entpCode) "
			, nativeQuery = true)
	int findQeenMobileTarget(String entpCode, String brandCode );
}
