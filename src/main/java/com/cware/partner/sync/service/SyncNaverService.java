...
import com.cware.partner.sync.repository.PaNaverGoodsRepository;
import com.cware.partner.sync.repository.PaNaverOriginMappingRepository;
...
@Slf4j
@Service
public class SyncNaverService extends PartnerSyncService {

	@Autowired
	private PaNaverOriginMappingRepository originMappingRepository;

	@Autowired
	private PaNaverGoodsRepository partnerGoodsRepository;
	...
	public boolean syncProduct(Goods goods, PaGoodsTarget target) {
		...
		target.setMediaCode(PaGroup.NAVER.mediaCode());

		log.info("{} -------> {}", tag, target);

		Optional<PaNaverGoods> optional = partnerGoodsRepository
				.findById(new PaGoodsId(target.getPaGroupCode(), target.getPaCode(), target.getGoodsCode()));

		PaNaverGoods partnerGoods;
		...
		// 신규입점
		partnerGoods = PaNaverGoods.builder().goodsCode(target.getGoodsCode())
				...
				.paGoodsName(goods.getGoodsName())
				.massTargetYn("0")
	...
	private boolean mappingOriginCode(String originCode, PaGoodsTarget target) {
		PaNaverGoods partnerGoods = (PaNaverGoods) target.getPartnerGoods();
		Optional<PaNaverOriginMapping> optional = originMappingRepository.findById(originCode);
		if (optional.isPresent()) {
			partnerGoods.setNaverOriginCode(optional.get().getNaverOriginCode());
		} else {
			partnerGoods.setNaverOriginCode("03"); // 상세설명에 표시
		}
	...
	private boolean syncPaGoods (Goods goods, PaGoodsTarget target) {
		...
		if (goods.getPaGoods().getLastSyncDate().after(partnerGoods.getLastSyncDate())) {
			mappingOriginCode(goods.getOriginCode(), target);
			partnerGoods.setPaGoodsName(goods.getGoodsName());
	...
	public int stopSale(String goodsCode, String paCode) {
		...
		log.info("네이버 판매중지 상품: {} ({})", goodsCode, result );
	...
	public boolean stopSale(PaGoodsTarget target) {
		...
		PaNaverGoods partnerGoods = optional.get();
		Timestamp currentDate = commonService.currentDate();
		...
		if (PaStatus.COMPLETE.code().equals(partnerGoods.getPaStatus()) &&
			...
			PaSaleNoGoods saleNo = PaSaleNoGoods.builder()
					...
					.paGoodsCode(partnerGoods.getProductId())
	...
	public boolean enableTransTarget(String goodsCode, String paCode) {
		...
		log.info("네이버 전송대상 상품: {} ({})", goodsCode, result );
