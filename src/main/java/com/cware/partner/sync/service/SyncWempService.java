...
import com.cware.partner.sync.domain.entity.PaWempGoods;
...
import com.cware.partner.sync.repository.PaWempGoodsRepository;

import lombok.extern.slf4j.Slf4j;

@Slf4j
@Service
public class SyncWempService extends PartnerSyncService {

	@Autowired
	private PaWempGoodsRepository partnerGoodsRepository;
	...
	public boolean syncProduct(Goods goods, PaGoodsTarget target) {
		...
		target.setMediaCode(PaGroup.WEMP.mediaCode());

		log.info("{} -------> {}", tag, target);

		Optional<PaWempGoods> optional = partnerGoodsRepository
				.findById(new PaGoodsId(target.getPaGroupCode(), target.getPaCode(), target.getGoodsCode()));
		PaWempGoods partnerGoods;
		...
		// 신규입점
		partnerGoods = PaWempGoods.builder().goodsCode(target.getGoodsCode())
		...
		// 브랜드매핑
		partnerGoods.setBrandNo(brandMappingRepository.findWempMapping(goods.getBrandCode()));
	...
	private boolean syncGoodsKindsModify(String lmsdCode, PaGoodsTarget target) {
        ...
        PaWempGoods partnerGoods = (PaWempGoods) target.getPartnerGoods();
	...
	public int stopSale(String goodsCode, String paCode) {
		...
		log.info("위메프 판매중지 상품: {} ({})", goodsCode, result );
	...
	public boolean stopSale(PaGoodsTarget target) {
		...
		PaWempGoods partnerGoods = optional.get();
		Timestamp currentDate = commonService.currentDate();
	...
	public boolean enableTransTarget(String goodsCode, String paCode) {
		...
		log.info("위메프 전송대상 상품: {} ({})", goodsCode, result );
