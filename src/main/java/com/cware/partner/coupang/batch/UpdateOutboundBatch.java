...
@Slf4j
@Service
public class UpdateOutboundBatch {
	...
	public void updateOutbound() {
		...
		transBatch.setBatchNote("쿠팡-출고지수정요청배치");
		transBatch.setStartDate(startDate);
		transBatch.setProcessId(Application.ID.code());
		batchRepository.save(transBatch);

		log.info("쿠팡 출고지수정 타겟팅 배치번호: {}", transBatch.getTransBatchNo());

		while (true) {

			// 쿠팡 출고지 수정 대상
			slice = entpSlipRepository.findUpdateOutboundTargetList(pageable);
			...
			for (PaEntpSlip entpSlip : targetList) {
				try {
					...
					int result = partnerSellerService.updateOutbound(entpSlip.getEntpCode(), entpSlip.getEntpManSeq(),
		...
		log.info("쿠팡 출고지수정 완료({}) 타겟:{} 성공:{} 필터:{} 실패:{} End ==> {}s elapsed.", transBatch.getTransBatchNo(), targetCnt,
