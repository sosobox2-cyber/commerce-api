...
@Slf4j
@Service
public class StopSaleProductBatch {
	...
	public void stopSaleProducts() {
		...
		Pageable pageable = PageRequest.of(0, PAGE_SIZE,
				Sort.by("paCode").and(Sort.by("modifyDate")).and(Sort.by("goodsCode")));
		...
		transBatch.setBatchNote("쿠팡-판매중지배치");
		transBatch.setStartDate(startDate);
		transBatch.setProcessId(Application.ID.code());
		batchRepository.save(transBatch);

		log.info("쿠팡 판매중지 타겟팅 배치번호: {}", transBatch.getTransBatchNo());

		while (true) {

			// 판매중지대상건
			slice = copnGoodsRepository.findStopSaleTargetList(pageable);
			...
			for (PaCopnGoods copnGoods : targetList) {
				...
				//  판매중지
				futures.add(saleProductService.asyncStopSaleProduct(copnGoods.getGoodsCode(), copnGoods.getPaCode(),
		...
		log.info("쿠팡 판매중지 완료({}) 타겟:{} 성공:{} 필터:{} 실패:{} End ==> {}s elapsed.", transBatch.getTransBatchNo(), targetCnt,
