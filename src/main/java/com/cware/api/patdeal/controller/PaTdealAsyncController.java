...
@Controller("com.cware.api.patdeal.PaTdealAsycController")
@RequestMapping(value="/patdeal/async")
public class PaTdealAsyncController extends AbstractController {
	...
	@Autowired
	@Qualifier("pacommon.claim.paclaimService")
	private PaClaimService paclaimService;
	...
	public void orderInputAsync(Map<String, String> order, HttpServletRequest request) throws Exception {
		...
		try {
			...
			orderInputTargetDtList = paTdealDeliveryService.selectOrderInputTargetDtList(paramMap);
			...
			PaTdealComUtil.replaceCamelList(orderInputTargetDtList);
			for(Map<String, Object> map : orderInputTargetDtList) {
				...
				if(receiverTel.length() < 9) {
					orderInputVO[index] = vo;
					throw processException("pa.fail_order_input", new String[]{ "전화번호가 9자 미만입니다." });
				}
				...
				String paOrderDate   = map.get("orderDate").toString(); // 티딜 주문일시
				...
				vo.setProcUser  (Constants.PA_TDEAL_PROC_ID);
			...
			paramMap = new ParamMap();
			paramMap.put("apiCode"	, "PATDEAL_ORDER_INPUT_ASYNC");
	...
	public void cancelInputAsync(HashMap<String, Object> cancelTarget, HttpServletRequest request) {
		...
		try {
			...
			HashMap<String, Object> cancelDtInfo = paTdealCancelService.selectCancelInputTargetDtList(paramMap);
			...
			case "0" :
				...
				cancelInputVO.setProcId	    (Constants.PA_TDEAL_PROC_ID);
			...
			default : //기취소
				...
				//paTdealDeliveryService.updatePreCanYn(preCancelMap);
			...
			paramMap.put("apiCode", "PATDEAL_CANCEL_INPUT_A");
	...
	public void returnClaimAsync(HashMap<String, Object> claimMap, HttpServletRequest request) {
		...
		try {
			...
			if(paramMap.getString("paOrderGb").equals("30")) {
				orderClaimTargetDtList = paTdealClaimService.selectOrderCalimTargetDt30List(paramMap); 
			} else {
				orderClaimTargetDtList = paTdealClaimService.selectOrderCalimTargetDt20List(paramMap);
			}
	...
	public void claimCancelAsync(HashMap<String, Object> claimMap, HttpServletRequest request) {
		...
		try {
			if(claimMap.get("PA_ORDER_GB").equals("31")) { // 반품철회
				cancelInputTargetDtList = paTdealClaimService.selectClaimCancelTargetDtList(paramMap); 
				...
				//티딜은 클레임철회 조회가 없으므로 기취소건이 발생 할 수 없다.
				orderClaimVO = setOrderClaimVO(hmSheet, isLocalYn);
				paclaimService.saveClaimCancelTx(orderClaimVO);	
			}else { // 교환 철회
				cancelInputTargetDtList = paTdealClaimService.selectChangeCancelTargetDtList(paramMap);
			...
			log.error(message);
			paramMap.put("code"		, "500");
			paramMap.put("apiCode"  , "PATDEAL_CLAIM_CANCEL_A");
			paramMap.put("message" 	, message);
	...
	private OrderClaimVO setOrderClaimVO(HashMap<String, Object> hmSheet, String isLocalYn) throws Exception{
		...
		case "30":
			...
			checkAddr = paTdealClaimService.compareAddress(paramMap);
		...
		//귀책자에 따른 shpfeeYn 결정
		String reason = String.valueOf(hmSheet.get("RESPONSIBLE_OBJECT_TYPE"));
		if("BUYER".equals(reason)) {
        ...
        // 출고전 반품케이스 제외, 전화번호 null인 경우 클레임 데이터 미생성 처리
        if( !("1").equals(orderClaimVO.getOutBefClaimGb()) &&
        		!("31").equals(hmSheet.get("PA_ORDER_GB").toString()) &&
        		!("41").equals(hmSheet.get("PA_ORDER_GB").toString()) &&
        		!("46").equals(hmSheet.get("PA_ORDER_GB").toString()) &&
        		!StringUtils.hasText(orderClaimVO.getReturnTel()) && !StringUtils.hasText(orderClaimVO.getReturnHp())){
	...
	private void refusalOrder(HttpServletRequest request, HashMap<String, Object>[] resultMap) throws Exception {
		...
		for(HashMap<String ,Object> map :  resultMap) {
			...
			try {
				// 취소할 주문 정보 조회
				cancelOrderMap = paTdealDeliveryService.selectRefusalInfo(mappingSeq); 
				
				if(!cancelOrderMap.isEmpty()) {
					String orderOptionNo = String.valueOf(cancelOrderMap.get("ORDER_PRODUCT_OPTION_NO"));
					String paCode = String.valueOf(cancelOrderMap.get("PA_CODE"));
					// 취소요청 api > 클레임 목록 조회 > 클레임 승인
					// 1. 취소 요청 api 
					paTdealCancelService.cancelRequest(cancelOrderMap, request); // 취소 요청 
					
					// 2. 취소 목록 조회 api
					List<PaTdealClaimListVO> paTdealCancelVOList = paTdealClaimService.getTdealClaimList("20", null, null, request);
					
					Optional<PaTdealClaimListVO>  result =  paTdealCancelVOList.stream().filter(x -> String.valueOf(x.getOrderOptionNo()).equals(orderOptionNo)).findAny();
					
					if (result.isPresent() == true) {
						String apiCode = "IF_PATDEALAPI_03_011"; 
						String pathParameters = String.valueOf(result.get().getClaimNo());  
						// Query Parameters 세팅
						...
						if(responseMap.get("status").equals("204") && !responseMap.containsKey("code")) {
							//tpaorderm 업데이트
							resultMsg  = "async 재고부족 주문취소 성공";
				...
				m.put("mappingSeq"		, mappingSeq);
				
				paTdealDeliveryDAO.updatePaorderm(m); 
			}
		}
	}
	...
	public void changeClaimAsync(HashMap<String, Object> claim, HttpServletRequest request) {
		...
		try {
			orderChangeTargetDtList = paTdealClaimService.selectOrderChangeTargetDtList(paramMap);
			...
			for(int j = 0; targetSize > j; j++) {
				...
				if(paramMap.getString("resultCode").equals("100001")) {
					for(OrderClaimVO claimVO : orderClaimVOArray) {
						paTdealClaimService.updatePaOrdermChangeFlag("06", claimVO.getMappingSeq());
						//티딜은 아웃바운드 처리로 교환 취소 할 수 밖에 없다.
					}
				}
			}
		} catch(Exception e) {
