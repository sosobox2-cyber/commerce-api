 ...
 */
@Service
public class Sk11stProductService {

	// 11번가데몬서버
	@Value("${partner.11st.service.host}")
	String API_SERVER;

	// 판매중지 API 
	@Value("${partner.11st.service.product.stop}")
	String PRODUCT_STOP_API;

	// 판매재개 API 
	@Value("${partner.11st.service.product.resume}")
	String PRODUCT_RESUME_API;

	// 재고번호매핑 API 
	@Value("${partner.11st.service.product.mapping}")
	String PRODUCT_MAPPING_API;

	// 재고변경 API 
	@Value("${partner.11st.service.product.stock}")
	 ...
	 * @return
	 */
	public PaTransService updateOptionProduct(String goodsCode, String paCode, String procId) {
		String tag = "재고번호매핑";
