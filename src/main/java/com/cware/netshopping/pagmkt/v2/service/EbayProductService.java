...
public class EbayProductService {
	...
	// 이베이 데몬서버
	@Value("${partner.ebay.service.host}")
	String API_SERVER;

	// 판매중지 API 
	@Value("${partner.ebay.service.product.stop}")
	String PRODUCT_STOP_API;

	// 판매기한연장 API 
	@Value("${partner.ebay.service.product.period}")
	String PRODUCT_PERIOD_API;
	 ...
	 * @return
	 */
	public PaTransService sellingPeriodProduct(String goodsCode, String paCode, String procId) {
		String tag = "판매기한연장";
		...
		return requestUpdateProduct(goodsCode, tag, PRODUCT_PERIOD_API, params);
	}
