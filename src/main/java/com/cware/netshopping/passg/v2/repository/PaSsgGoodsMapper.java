...
@Mapper
public interface PaSsgGoodsMapper {
	...
	PaSsgGoodsVO getGoods(@Param("goodsCode") String goodsCode, @Param("paCode") String paCode);
     ...
     * @return
     */
    @Update(value = "update tpassggoods p "
    		+ " set p.pa_status = '15' "
    ...
    @Update(value = "update tpassggoods p "
    		...
    		+ " where p.goods_code = #{goodsCode} "
    		+ "   and p.pa_code = #{paCode} ")
	int updateResetTrans(PaSsgGoodsVO ssgGoods);
    ...
    @Update(value = "update tpassggoods p "
    		...
    		+ " where p.goods_code = #{goodsCode} "
    		+ "   and p.pa_code = #{paCode} ")
	int updateResetSaleTrans(PaSsgGoods ssgGoods);
     ...
     */
    @Update(value = "update tpassggoods p "
    		+ " set p.goods_name = #{goodsName} "
    		+ " , p.pa_status = '10' "
    		+ " , p.modify_id = #{modifyId} "
    		+ " , p.modify_date = sysdate "
    		+ " where p.goods_code = #{goodsCode} "
    		+ "   and p.pa_code = #{paCode} ")
	int updateGoodsName(@Param("goodsCode") String goodsCode, @Param("paCode") String paCode,
			@Param("modifyId") String modifyId, @Param("goodsName") String goodsName);
     ...
     */
    @Update(value = "update tpassggoods p "
            + " set p.pa_collect_yn = #{collectYn} "
            + " , p.modify_id = #{modifyId} "
            + " , p.modify_date = sysdate "
            + " where p.goods_code = #{goodsCode} "
            + "   and p.pa_code = #{paCode} ")
    int updatePaCollectYn(PaSsgGoods ssgGoods);
