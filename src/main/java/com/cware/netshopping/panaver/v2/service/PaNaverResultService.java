...
@Service
public class PaNaverResultService {
...
//	private Logger log = LoggerFactory.getLogger(this.getClass());
	...
	public String saveTransProduct(PaNaverGoodsVO paNaverGoods, PaGoodsPriceApply priceApply) {
		String rtnMsg = Constants.SAVE_SUCCESS;
 	    ...
 	    // 네이버 전송 업데이트
 	   	paNaverGoodsMapper.updateResetTrans(paNaverGoods);
	...
	public String saveTransProductOption(List<PaGoodsdtMapping> goodsOptionList, String goodsCode, String paCode, String procId) {
		...
		// 전송 비대상 단품 리셋
		goodsdtMappingMapper.updateResetTrans(goodsCode, paCode, procId);
	 ...
	 */
	@Transactional
	public String saveTransImage(PaNaverGoodsImage image) {
		String rtnMsg = Constants.SAVE_SUCCESS;
		
		// 네이버이미지 URL 저장
		naverGoodsImageMapper.mergeCompleteTrans(image);

		// 상품이미지 전송 완료
		goodsImageMapper.updateCompleteTrans(image.getGoodsCode(), PaGroup.NAVER.code(), image.getModifyId());
