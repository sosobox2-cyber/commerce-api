...
public class CoupangProductService {
	...
	// 쿠팡데몬서버
	@Value("${partner.coupang.service.host}")
	String API_SERVER;

	// 상태업데이트 API 
	@Value("${partner.coupang.service.product.status}")
	String PRODUCT_STATUS_API;

	// 가격변경 API 
	@Value("${partner.coupang.service.product.price}")
	String PRODUCT_PRICE_API;

	// 판매중지 API 
	@Value("${partner.coupang.service.product.stop}")
	String PRODUCT_STOP_API;

	// 판매재개 API 
	@Value("${partner.coupang.service.product.resume}")
	String PRODUCT_RESUME_API;

	// 재고변경 API 
	@Value("${partner.coupang.service.product.stock}")
	 ...
	 * @return
	 */
	public PaTransService updatePriceProduct(String goodsCode, String paCode, String procId) {
		String tag = "가격변경";
		...
		return requestUpdateProduct(goodsCode, tag, PRODUCT_PRICE_API, params);
	}
