...
class DescribeRepositoryTest {
	...
	void test() {
		...
		paGroupCodes.add("05");
		Slice<GoodsTarget> goodsSlice = describeRepository.findDescribeModifyTargetList(LAST_CDC_DATE, 0, pageable, paGroupCodes);
    ...
    void testNativeQuery() {
        ...
        Slice<GoodsTarget> goodsSlice = describeRepository.findDescribeModifyTargetList(LAST_CDC_DATE, 0, 10000, PA_GROUPS);
        System.out.println(goodsSlice.getContent().size());
        assertThat(goodsSlice.getSize()).isGreaterThan(0);
    }
}
