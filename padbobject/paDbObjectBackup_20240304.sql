...
-- Created by hham3 on 2024-03-04, 오전 10:08:51 --
-------------------------------------------------

set define off
spool paDbObjectBackup_20240304.log
  ...
  SELECT CASE
    ...
    FROM (SELECT CL.ORDER_NO,
           ...
           WHERE CL.MEDIA_GB = '03'
             ...
             AND CL.CLAIM_DATE >= TO_DATE(V_FROMDATE, 'YYYYMMDDHH24MISS')
             AND CL.CLAIM_GB = '40'
             AND CL.ORDER_NO NOT IN ('20240122149828', -- 2/15
                                     '20240209540393',  --2/19
                                     '20240124931750'  --2/22
                                     )
             AND NOT EXISTS
  ...
  SELECT CASE
    ...
    FROM ( SELECT HE.ORD_NO AS PA_ORDER_NO
            ...
            WHERE HE.INSERT_DATE > SYSDATE - 3
              AND HE.ORD_NO NOT IN ('240111624498','240201835589', '240201867891'));
  DBMS_OUTPUT.PUT_LINE('V_FLAG : ' || V_FLAG || ', TIME : ' || TO_CHAR(SYSDATE,'YYYY/MM/DD HH24:MI:SS'));
  IF (IS_EXISTED > 0) THEN
    INSERT_ERROR_LOG();
  END IF;  
  ...
  SELECT CASE
    ...
    FROM (SELECT * 
           ...
           WHERE OD.ORDER_DATE > SYSDATE - 1
             ...
             AND OD.ORDER_NO NOT IN ('20231130098656'));
  DBMS_OUTPUT.PUT_LINE('V_FLAG : ' || V_FLAG || ', TIME : ' || TO_CHAR(SYSDATE,'YYYY/MM/DD HH24:MI:SS'));
  IF (IS_EXISTED > 0) THEN
    INSERT_ERROR_LOG();
  END IF; 
