...
-- Created by hham3 on 2023-03-08, 오전 9:39:10 --
------------------------------------------------

set define off
spool paDbObjectBackup_20230308.log
                 ...
                 AND ( GPA.APPLY_DATE, GPA.PRICE_APPLY_SEQ ) IN ( SELECT /*+ INDEX_DESC (PP PK_TPAGOODSPRICEAPPLY)*/
                                                                  ...
                                                                  WHERE PP.GOODS_CODE = GPA.GOODS_CODE
                                                                    ...
                                                                    AND PP.APPLY_DATE < SYSDATE
                                                                     AND ROWNUM = 1 )
                  AND 1 = CASE WHEN GPA.PA_CODE IN ('B1','B2') THEN ( SELECT 1 FROM TPAKAKAOGOODS PG WHERE PG.GOODS_CODE = GPA.GOODS_CODE AND PG.PA_CODE = GPA.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' AND PG.PA_GROUP_CODE = GPA.PA_GROUP_CODE)
                               WHEN GPA.PA_CODE IN ('C1','C2') THEN ( SELECT 1 FROM TPAHALFGOODS PG WHERE PG.GOODS_CODE = GPA.GOODS_CODE AND PG.PA_CODE = GPA.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' AND PG.PA_GROUP_CODE = GPA.PA_GROUP_CODE)
           ...
           WHERE OL.INSERT_DATE BETWEEN TO_DATE(V_TODATE, 'yyyymmdd') - 2 AND
             ...
             AND NOT EXISTS (SELECT 1
                   ...
                   WHERE PM.PA_GROUP_CODE = '01'
                     ...
                     AND PM.PA_ORDER_NO = OL.ORD_NO
                     AND ROWNUM = 1)
             AND OL.ORD_NO NOT IN ('20221207576130882' -- 12/8
    ...
    FROM (SELECT PA_ORDER_GB, PA_SHIP_NO
          ...
          --반품처리완료.
           WHERE PA_SHIP_NO NOT IN ('G1182236034' -- 12/6 주문생성전 취소
                                   ...
                                   ,'G1223261026' -- 2/9 주문생성전 취소
                                   ,'G1230711818' -- 2/22 주문생성전 취소
                                    )
  ...
  SELECT CASE
    ...
    FROM ( SELECT HE.ORD_NO AS PA_ORDER_NO
            ...
            WHERE HE.INSERT_DATE > SYSDATE - 3
              AND HE.ORD_NO NOT IN  ('230206454780','230215104248','230213960415','230215043553','230213966978'));
  IF (IS_EXISTED > 0) THEN
    INSERT_ERROR_LOG();
  END IF;  
  ...
  SELECT CASE
    ...
    FROM (SELECT * 
           ...
           WHERE OD.ORDER_DATE > SYSDATE - 1
             ...
             AND OD.ORDER_NO NOT IN ('20230306559959'));   
  IF (IS_EXISTED > 0) THEN
    INSERT_ERROR_LOG();
  END IF; 
