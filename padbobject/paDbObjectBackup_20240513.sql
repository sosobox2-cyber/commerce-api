...
-- Created by hham3 on 2024-05-13, 오후 1:39:08 --
------------------------------------------------

set define off
spool paDbObjectBackup_20240513.log
...
prompt
prompt Creating procedure SP_PAGOODS_RETENTION
prompt =======================================
prompt
CREATE OR REPLACE PROCEDURE BMTCOM.SP_PAGOODS_RETENTION(
    arg_pa_group_code   IN   VARCHAR2
   , arg_goods_code      IN   VARCHAR2
   , arg_status              IN   VARCHAR2
   , rtn_code               OUT INTEGER
...
/***********************************************************************
    ...
    Job     : 제휴 상품 공통 리텐션 로직
    Date    : 2024/01/24
    Creator : CommerceWare (EJPARK, JUNGYJ)

    INPUT :
        - arg_pa_group_code     : 그룹코드 (EX - 01:11ST, 02: 이베이 ...)
        - arg_goods_code        : 상품코드
        - arg_status               :  U : 입점상태로 초기화, D : 제휴테이블에서 모두 삭제
    ...
    STEP  :
        - Step1. 리텐션 대상 데이터 SELECT (TPARETENTIONTARGET)
        - Step2. 주문 이력 존재 여부 CHECK
        - Step3. 공통 테이블 DELETE
        - Step4. 제휴사별 테이블 DELETE
        - Step5. 리텐션 대상 데이터 DELETE (TPARETENTIONTARGET)
        - Step6. 리텐션 대상 히스토리 INSERT (TPARETENTIONGOODS)
...
v_fail_msg          VARCHAR2(3000);

v_sysdate         VARCHAR2(8);
v_sysdatetime     VARCHAR2(14);
v_user_id         VARCHAR2(10);
i_proc_cnt        INTEGER := 0;
i_ok_cnt          INTEGER := 0;
i_fail_cnt        INTEGER := 0;
i_total_fail_cnt  INTEGER := 0;
v_proc_msg        VARCHAR2(1000) := '';

v_chk_order_cnt INTEGER := 0;
v_retention_sql VARCHAR2(500);
v_goods_code VARCHAR2(10);


/*########## CURSOR START ##########*/

TYPE RETENTION_TARGET_REC IS RECORD (
     v_pa_code          BMTPARTNER.TPAGOODSTARGET.PA_CODE%TYPE
   , v_pa_group_code    BMTPARTNER.TPAGOODSTARGET.PA_GROUP_CODE%TYPE
   , v_goods_code       BMTPARTNER.TPAGOODSTARGET.GOODS_CODE%TYPE
   , v_status             BMTPARTNER.TPARETENTIONTARGET.STATUS%TYPE
);
retention_target RETENTION_TARGET_REC;

TYPE retention_curor_type IS REF CURSOR;
CUR_GOODS_RETENTION retention_curor_type;
...
-- 공통 --
PROCEDURE SELECT_ORDER_DATA_CHECK(arg_goods_code IN VARCHAR2, arg_pa_group_code IN VARCHAR2) IS
  BEGIN

      IF arg_pa_group_code = '02' THEN
         SELECT COUNT(DISTINCT OD.ORDER_NO)
           INTO v_chk_order_cnt
           FROM BMTCOM.TORDERDT OD
          WHERE OD.GOODS_CODE   = arg_goods_code
            AND OD.ORDER_DATE >= SYSDATE - INTERVAL '6' MONTH
            AND OD.MEDIA_CODE   IN ('EX02', 'EX03')
          ;
      ELSE
         SELECT COUNT(DISTINCT OD.ORDER_NO)
           INTO v_chk_order_cnt
          ...
          WHERE OD.GOODS_CODE   = arg_goods_code
            AND OD.ORDER_DATE   >= SYSDATE - INTERVAL '6' MONTH
            AND OD.MEDIA_CODE     = (SELECT REMARK1
                                                 FROM TCODE TC
                                                WHERE TC.CODE_LGROUP = 'O500'
                                                   AND TC.CODE_MGROUP = arg_pa_group_code)
          ;
      END IF;
  EXCEPTION
   WHEN NO_DATA_FOUND THEN
...
PROCEDURE DELETE_PA_GOODS_TARGET IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAGOODSTARGET PGT
         WHERE PGT.GOODS_CODE    = retention_target.v_goods_code
           AND PGT.PA_GROUP_CODE = retention_target.v_pa_group_code
           AND PGT.PA_CODE       = retention_target.v_pa_code;
    END;

PROCEDURE DELETE_PA_GOODS_PRICE IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAGOODSPRICE PGP
         WHERE PGP.GOODS_CODE = retention_target.v_goods_code
           AND PGP.PA_CODE    = retention_target.v_pa_code
        ;
    END;

PROCEDURE DELETE_PA_GOODS_OFFER IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAGOODSOFFER PGO
         WHERE PGO.GOODS_CODE     = retention_target.v_goods_code
           AND PGO.PA_GROUP_CODE  = retention_target.v_pa_group_code
        ;
    END;

PROCEDURE DELETE_PA_GOODSDT_MAPPING IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAGOODSDTMAPPING PDM
         WHERE PDM.GOODS_CODE = retention_target.v_goods_code
           AND PDM.PA_CODE    = retention_target.v_pa_code
        ;
    END;

PROCEDURE DELETE_PA_GOODS_IMAGE IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAGOODSIMAGE PGI
         WHERE PGI.GOODS_CODE     = retention_target.v_goods_code
           AND PGI.PA_GROUP_CODE  = retention_target.v_pa_group_code
        ;
    END;

PROCEDURE DELETE_PA_GOODSPRICE_APPLY IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAGOODSPRICEAPPLY PP
         WHERE PP.GOODS_CODE    = retention_target.v_goods_code
           AND PP.PA_GROUP_CODE = retention_target.v_pa_group_code
           AND PP.PA_CODE       = retention_target.v_pa_code
        ;
    END;

PROCEDURE DELETE_PA_PROMO_TARGET IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAPROMOTARGET PPT
         WHERE PPT.GOODS_CODE    = retention_target.v_goods_code
           AND PPT.PA_GROUP_CODE = retention_target.v_pa_group_code
           AND PPT.PA_CODE       = retention_target.v_pa_code
        ;
    END;

PROCEDURE INSERT_PA_RETENTION_GOODS(in_memo IN VARCHAR2) IS
    BEGIN
        INSERT INTO BMTPARTNER.TPARETENTIONGOODS
                      (PA_CODE,
                       PA_GROUP_CODE,
                       GOODS_CODE,
                       STATUS,
               ...
               VALUES (retention_target.v_pa_code,
                       retention_target.v_pa_group_code,
                       retention_target.v_goods_code,
                       retention_target.v_status,
                       (SELECT NVL(LPAD(TO_CHAR(MAX(TO_NUMBER(PRG.SEQ)) + 1), 5, '0'), '00001')
                          FROM BMTPARTNER.TPARETENTIONGOODS PRG
                         WHERE PRG.PA_CODE       = retention_target.v_pa_code
                           AND PRG.PA_GROUP_CODE = retention_target.v_pa_group_code
                           AND PRG.GOODS_CODE    = retention_target.v_goods_code),
                       in_memo,
                       SYSDATE,
                       'RETENTION',
                       '0'
...
PROCEDURE INSERT_PA_GOODS_TAGET IS
    BEGIN
        INSERT INTO BMTPARTNER.TPAGOODSTARGET
                      (PA_GROUP_CODE,
                       PA_CODE,
                       GOODS_CODE,
                       INSERT_ID,
               ...
               VALUES (retention_target.v_pa_group_code,
                       retention_target.v_pa_code,
                       retention_target.v_goods_code,
                       'RETENTION',
                       SYSDATE,
                       'RETENTION',
...
PROCEDURE DELETE_PA_RETENTION_TARGET IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPARETENTIONTARGET PRT
         WHERE PRT.GOODS_CODE    = retention_target.v_goods_code
           AND PRT.PA_GROUP_CODE = retention_target.v_pa_group_code
        ;
    END;

-- 11ST --
PROCEDURE DELETE_11ST_GOODS IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPA11STGOODS PG
         WHERE PG.GOODS_CODE = retention_target.v_goods_code
           AND PG.PA_CODE    = retention_target.v_pa_code
        ;
    END;

-- EBAY --
PROCEDURE DELETE_EBAY_GOODS_TARGET IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAGOODSTARGET PGT
         WHERE PGT.GOODS_CODE    = retention_target.v_goods_code
           AND PGT.PA_GROUP_CODE IN ('02', '03')
           AND PGT.PA_CODE       = retention_target.v_pa_code
        ;
    END;

PROCEDURE DELETE_EBAY_GOODS IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAGMKTGOODS PG
         WHERE PG.GOODS_CODE = retention_target.v_goods_code
           AND PG.PA_CODE    = retention_target.v_pa_code
        ;
    END;

PROCEDURE DELETE_EBAY_GOODSPRICE_APPLY IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAGOODSPRICEAPPLY PP
         WHERE PP.GOODS_CODE    = retention_target.v_goods_code
           AND PP.PA_GROUP_CODE IN ('02', '03')
           AND PP.PA_CODE       = retention_target.v_pa_code
        ;
    END;

PROCEDURE DELETE_EBAY_PROMO_TARGET IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAPROMOTARGET PPT
         WHERE PPT.GOODS_CODE    = retention_target.v_goods_code
           AND PPT.PA_GROUP_CODE IN ('02', '03')
           AND PPT.PA_CODE       = retention_target.v_pa_code
        ;
    END;

-- NAVER --
PROCEDURE DELETE_NAVER_GOODS IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPANAVERGOODS PG
         WHERE PG.GOODS_CODE = retention_target.v_goods_code
           AND PG.PA_CODE    = retention_target.v_pa_code
        ;
    END;

PROCEDURE DELETE_NAVER_GOODSNAME IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPANAVERGOODSNAME NGN
         WHERE NGN.GOODS_CODE = retention_target.v_goods_code
        ;
    END;

-- COPN --
PROCEDURE DELETE_COPN_GOODS IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPACOPNGOODS PG
         WHERE PG.GOODS_CODE = retention_target.v_goods_code
           AND PG.PA_CODE    = retention_target.v_pa_code
        ;
    END;

PROCEDURE DELETE_COPN_GOODSATTRI IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPACOPNGOODSATTRI CGA
         WHERE CGA.GOODS_CODE = retention_target.v_goods_code
        ;
    END;

-- WEMP --
PROCEDURE DELETE_WEMP_GOODS IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAWEMPGOODS PG
         WHERE PG.GOODS_CODE = retention_target.v_goods_code
           AND PG.PA_CODE    = retention_target.v_pa_code
        ;
    END;
...
-- INTP --
PROCEDURE DELETE_INTP_GOODS IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAINTPGOODS PG
         WHERE PG.GOODS_CODE = retention_target.v_goods_code
           AND PG.PA_CODE    = retention_target.v_pa_code
        ;
    END;
...
-- LTON --
PROCEDURE DELETE_LTON_GOODS IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPALTONGOODS PG
         WHERE PG.GOODS_CODE = retention_target.v_goods_code
           AND PG.PA_CODE    = retention_target.v_pa_code
        ;
    END;

PROCEDURE DELETE_LTON_GOODSDT_MAPPING IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPALTONGOODSDTMAPPING LDM
         WHERE LDM.GOODS_CODE = retention_target.v_goods_code
           AND LDM.PA_CODE    = retention_target.v_pa_code
        ;
    END;

-- TMON --
PROCEDURE DELETE_TMON_GOODS IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPATMONGOODS PG
         WHERE PG.GOODS_CODE = retention_target.v_goods_code
           AND PG.PA_CODE    = retention_target.v_pa_code
        ;
    END;
...
-- SSG --
PROCEDURE DELETE_SSG_GOODS IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPASSGGOODS PG
         WHERE PG.GOODS_CODE = retention_target.v_goods_code
           AND PG.PA_CODE    = retention_target.v_pa_code
        ;
    END;

PROCEDURE DELETE_SSG_GOODSDT_MAPPING IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPASSGGOODSDTMAPPING SDM
         WHERE SDM.GOODS_CODE = retention_target.v_goods_code
           AND SDM.PA_CODE    = retention_target.v_pa_code
        ;
    END;

PROCEDURE DELETE_SSG_GOODS_PRICE IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAPROMOGOODSPRICE PGP
         WHERE PGP.GOODS_CODE = retention_target.v_goods_code
           AND PGP.PA_CODE    = retention_target.v_pa_code
        ;
    END;
...
-- KAKAO --
PROCEDURE DELETE_KAKAO_GOODS IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAKAKAOGOODS PG
...
-- HALF --
PROCEDURE DELETE_HALF_GOODS IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAHALFGOODS PG
         WHERE PG.GOODS_CODE = retention_target.v_goods_code
           AND PG.PA_CODE    = retention_target.v_pa_code
        ;
    END;

PROCEDURE DELETE_HALF_GOODSDT_MAPPING IS
    BEGIN
        DELETE
          FROM BMTPARTNER.TPAHALFGOODSDTMAPPING HDM
         WHERE HDM.GOODS_CODE = retention_target.v_goods_code
           AND HDM.PA_CODE    = retention_target.v_pa_code
        ;
    END;
...
BEGIN
    -- setting program id --
    v_prog_id := 'sp_pagoods_retention';

    -- setting sysdate --
    rtn_code      := 999;
    rtn_msg       := 'Success';
    v_fail_msg     := '대상내역이 없습니다.';
...
-------------------------------------------------------------------------------
 ...
 /* Validation Check */
    IF (arg_pa_group_code IS NOT NULL OR arg_goods_code IS NOT NULL OR arg_status IS NOT NULL)  THEN
       IF arg_pa_group_code IS NULL THEN
          v_fail_msg :='pa_group_code를 입력하세요.';
       ELSIF arg_goods_code IS NULL THEN
          v_fail_msg :='goods_code를 입력하세요.';
       ELSIF arg_status IS NULL THEN
          v_fail_msg :='status를 입력하세요. (U: 초기화, D: 삭제)';
        END IF;
    END IF;

    PKG_COMMON.EXP_PRNS(v_prog_id, 'GOODS RETENTION PROCESS');

    --  Step1. 리텐션 대상 데이터 SELECT (TPARETENTIONTARGET),  --
    IF (arg_pa_group_code IS NOT NULL AND arg_goods_code IS NOT NULL AND arg_status IS NOT NULL) THEN
      v_retention_sql := 'SELECT PGT.PA_CODE, PGT.PA_GROUP_CODE, PGT.GOODS_CODE, ' || CHR(39) || arg_status || CHR(39) ||
                          ' FROM BMTPARTNER.TPAGOODSTARGET PGT
                           WHERE PGT.GOODS_CODE = ' || CHR(39) || arg_goods_code || CHR(39) ||
                           ' AND PGT.PA_GROUP_CODE = ' || CHR(39) || arg_pa_group_code || CHR(39);
    ELSE
      v_retention_sql := 'SELECT PGT.PA_CODE, PGT.PA_GROUP_CODE, PGT.GOODS_CODE, PRT.STATUS
                            FROM BMTPARTNER.TPARETENTIONTARGET PRT
                                ,BMTPARTNER.TPAGOODSTARGET PGT
                           WHERE PRT.GOODS_CODE = PGT.GOODS_CODE
                             AND PRT.PA_GROUP_CODE = PGT.PA_GROUP_CODE';
    END IF;

    v_fail_msg :='';

    OPEN CUR_GOODS_RETENTION FOR v_retention_sql;
         LOOP
           FETCH CUR_GOODS_RETENTION INTO retention_target;
            EXIT WHEN CUR_GOODS_RETENTION%NOTFOUND ;
                PKG_COMMON.EXP_PRNS(v_prog_id, 'GOODS RETENTION PROCESS Target GOODS_CODE :'||retention_target.v_goods_code);
                v_goods_code := retention_target.v_goods_code;
                rtn_code := 0;

                BEGIN

                    --  Step2. 주문 이력 존재 여부 CHECK --
                    SELECT_ORDER_DATA_CHECK(retention_target.v_goods_code, retention_target.v_pa_group_code);

                    IF v_chk_order_cnt = 0 THEN -- 주문 이력 없을 경우 리텐션 처리

                      -- Step3. 공통 테이블 DELETE --
                      IF retention_target.v_pa_group_code = '02' THEN -- EBAY
                        DELETE_PA_GOODS_PRICE();
                        DELETE_PA_GOODS_OFFER();
                        DELETE_PA_GOODSDT_MAPPING();
                        DELETE_PA_GOODS_IMAGE();
                      ELSIF retention_target.v_pa_group_code = '08' OR
                            retention_target.v_pa_group_code = '10' OR
                            retention_target.v_pa_group_code = '12' THEN -- LTON, SSG, HALF
                        DELETE_PA_GOODS_TARGET();
                        DELETE_PA_GOODS_PRICE();
                        DELETE_PA_GOODS_OFFER();
                        DELETE_PA_GOODS_IMAGE();
                        DELETE_PA_GOODSPRICE_APPLY();
                        DELETE_PA_PROMO_TARGET();
                      ELSE
                        DELETE_PA_GOODS_TARGET();
                        DELETE_PA_GOODS_PRICE();
                        DELETE_PA_GOODS_OFFER();
                      ...
                      -- Step4. 제휴사별 테이블 DELETE --
                      IF retention_target.v_pa_group_code = '01' THEN -- 11ST
                        DELETE_11ST_GOODS();
                      ELSIF retention_target.v_pa_group_code = '02'THEN -- EBAY
                        DELETE_EBAY_GOODS_TARGET();
                        DELETE_EBAY_GOODSPRICE_APPLY();
                        DELETE_EBAY_PROMO_TARGET();
                        DELETE_EBAY_GOODS();
                      ELSIF retention_target.v_pa_group_code = '04'THEN -- NAVER
                        DELETE_NAVER_GOODS();
                        DELETE_NAVER_GOODSNAME();
                      ELSIF retention_target.v_pa_group_code = '05'THEN -- COPN
                        DELETE_COPN_GOODS();
                        DELETE_COPN_GOODSATTRI();
                      ELSIF retention_target.v_pa_group_code = '06'THEN -- WEMP
                        DELETE_WEMP_GOODS();
                      ELSIF retention_target.v_pa_group_code = '07'THEN -- INTP
                        DELETE_INTP_GOODS();
                      ELSIF retention_target.v_pa_group_code = '08'THEN -- LTON
                        DELETE_LTON_GOODSDT_MAPPING();
                        DELETE_LTON_GOODS();
                      ELSIF retention_target.v_pa_group_code = '09'THEN -- TMON
                        DELETE_TMON_GOODS();
                      ELSIF retention_target.v_pa_group_code = '10'THEN -- SSG
                        DELETE_SSG_GOODSDT_MAPPING();
                        DELETE_SSG_GOODS_PRICE();
                        DELETE_SSG_GOODS();
                      ELSIF retention_target.v_pa_group_code = '11'THEN -- KAKAO
                        DELETE_KAKAO_GOODS();
                      ELSIF retention_target.v_pa_group_code = '12'THEN -- HALF
                        DELETE_HALF_GOODSDT_MAPPING();
                        DELETE_HALF_GOODS();
                      END IF;

                      IF retention_target.v_status = 'U' THEN -- 재입점
                        INSERT_PA_GOODS_TAGET();
                      END IF;

                      -- Step5. 리텐션 대상 데이터 DELETE (TPARETENTIONTARGET) --
                      DELETE_PA_RETENTION_TARGET();

                      -- Step6. 리텐션 대상 히스토리 INSERT (TPARETENTIONGOODS) --
                      INSERT_PA_RETENTION_GOODS('리텐션처리완료');

                    ELSIF v_chk_order_cnt > 0 THEN -- 주문 이력 있을 경우 타겟 OFF
                      IF arg_goods_code IS NULL THEN
                         DELETE_PA_RETENTION_TARGET();
                         INSERT_PA_RETENTION_GOODS('리텐션처리실패(주문이력존재)');
                      END IF;

                      i_fail_cnt := i_fail_cnt+1;
                      rtn_code := -1;
                      v_fail_msg := v_fail_msg || '[ Error - GOODS_CODE : ' || retention_target.v_goods_code ||' / Error Msg : '  || '최근 6개월동안 주문이력이 존재합니다.';
                      PKG_COMMON.PRNS(v_prog_id, v_fail_msg);
                      COMMIT;
                    ELSE -- 오류 발생
                      i_fail_cnt := i_fail_cnt+1;
                      v_fail_msg := v_fail_msg || '[ Error - GOODS_CODE : ' || retention_target.v_goods_code ||' / Error Msg : '  || rtn_code || SQLERRM || ' ';
                      PKG_COMMON.PRNS(v_prog_id, v_fail_msg);
                    END IF;
                ...
                IF rtn_code = 0 THEN
                    ...
                    i_ok_cnt := i_ok_cnt+1;
                END IF;
                i_proc_cnt := i_proc_cnt + 1;
         END LOOP;
    CLOSE CUR_GOODS_RETENTION
    ;

    v_proc_msg := '[ RETENTION Target : ' || i_proc_cnt || ' / Success : ' || i_ok_cnt || ' / Fail : ' || i_fail_cnt || ' / Massage : ' || v_fail_msg ||' ]';
    i_total_fail_cnt := i_total_fail_cnt+i_fail_cnt;
    PKG_COMMON.EXP_PRNS(v_prog_id, v_proc_msg);

    -- result --
    PKG_COMMON.EXP_PRNS(v_prog_id, rtn_msg);

    IF i_total_fail_cnt = 0 THEN
        rtn_code      := 0;
        rtn_msg       := 'Success ' || v_proc_msg;
    ELSE
        IF i_proc_cnt >= 1 THEN
          rtn_code      := 999;
          rtn_msg       := 'Fail ' || v_proc_msg;
        END IF;
    END IF;

    IF rtn_code = 0 THEN
        PKG_COMMON.HISTORY_SUCCESS(v_prog_id, v_user_id, v_sysdate, '', rtn_msg);
    ELSE
        PKG_COMMON.HISTORY_FAIL(v_prog_id, v_user_id, v_sysdate, '', rtn_msg);
    END IF;
    UTL_FILE.FCLOSE_ALL;
...
WHEN OTHERS THEN
     ...
     rtn_msg := v_db_error_msg || '. ERROR_CODE = ' || v_goods_code || v_chk_order_cnt;
     UTL_FILE.FCLOSE_ALL;

     IF CUR_GOODS_RETENTION%ISOPEN THEN CLOSE CUR_GOODS_RETENTION; END IF;
     PKG_COMMON.HISTORY_FAIL(v_prog_id, v_user_id, v_sysdate, '',  TRIM(v_db_error_msg));
END;
/
