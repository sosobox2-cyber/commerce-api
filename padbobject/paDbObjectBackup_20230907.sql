...
-- Created by hham3 on 2023-09-07, 오후 5:18:13 --
------------------------------------------------

set define off
spool paDbObjectBackup_20230907.log
            ...
            SELECT GD.GOODS_CODE
             ...
             WHERE PG.GOODS_CODE = GD.GOODS_CODE
               ...
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE 'C%' THEN NVL(( SELECT 1 FROM TPAHALFGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE 'A%' THEN NVL(( SELECT 1 FROM TPASSGGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END 
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '9%' THEN NVL(( SELECT 1 FROM TPATMONGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END    
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '8%' THEN NVL(( SELECT 1 FROM TPALTONGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '7%' THEN NVL(( SELECT 1 FROM TPAINTPGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '6%' THEN NVL(( SELECT 1 FROM TPAWEMPGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '5%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END                         
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '4%' THEN NVL(( SELECT 1 FROM TPANAVERGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '2%' THEN NVL(( SELECT 1 FROM TPAGMKTGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '1%' THEN NVL(( SELECT 1 FROM TPA11STGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END  
               AND GD.GOODS_CODE NOT IN ('23976603','32815224')
            ...
            SELECT G.GOODS_CODE
             ...
             WHERE G.MODIFY_DATE > SYSDATE - 1
               ...
               AND G.GOODS_CODE NOT IN ('25677110','25677126','25677213','25679058','25679101','25679139','25679178','25679186','25697949','25697958',
                                        ...
                                        , '29441229', '29929539', '31163941', '31423333', '31254681', '30355824','35123034' -- wemp 수기연동
      ...
      FROM (
        SELECT DECODE (IE.ROAD_ADDR_YN,'1', IE.ROAD_ADDR, IE.ADDR)
         ...
         WHERE A.PA_SHIP_COST_CODE   = B.PA_SHIP_COST_CODE
           ...
           AND (A.PA_SHIP_COST_CODE, A.SEQ)  = (SELECT B.PA_SHIP_COST_CODE , MAX(B.SEQ)
    ...
    FROM (SELECT CD.ORDER_NO,
           ...
           WHERE CD.CANCEL_DATE >= TO_DATE(V_FROMDATE, 'YYYYMMDDHH24MISS')
             ...
             AND CD.ORDER_NO NOT IN ('20220522472419' -- 5/26
                                    ,'20230427408945'  --4/28 테스트주문
                                    ,'20230814824361' --8/14 테스트주문 
                                     )
             AND NOT EXISTS
           (SELECT 1
                    FROM TPAORDERM PM
...
/*
  ...
  SELECT CASE
   ...
   WHERE ((HP IS NULL AND TEL = '000000000000') OR
     ...
     AND CUST_NO NOT IN ('202112132848' -- 12/3
                                    ,'202201877900' -- 23.04.27
                                    ,'202209637223' -- 23.06.09
                                    ,'202002769759' -- 23.06.20
                                    ,'202208359741'
                                    ,'202212506082'
                                    ,'202301017224' -- 23.07.12
                                    ,'202209672166' -- 23.07.19
                                    ,'202207414240' -- 23.08.04
                         );
  ...
  SELECT CASE
    ...
    FROM (SELECT DISTINCT PM.PA_ORDER_NO, PM.ORDER_NO
            ...
            WHERE PM.PA_CODE IN ('C1', 'C2')
             ...
             AND HO.ORD_NO NOT IN ('230614477432'));
  IF (IS_EXISTED > 0) THEN
    INSERT_ERROR_LOG();
  END IF;
  ...
  SELECT CASE
    ...
    FROM ( SELECT HE.ORD_NO AS PA_ORDER_NO
            ...
            WHERE HE.INSERT_DATE > SYSDATE - 3
              AND HE.ORD_NO NOT IN  ('230808283916', '230813509852'));
  IF (IS_EXISTED > 0) THEN
    INSERT_ERROR_LOG();
  END IF;  
  ...
  SELECT CASE
    ...
    FROM (SELECT '01' AS PA_GROUP_CODE, OL.ORD_NO
          ...
          SELECT '11' AS PA_GROUP_CODE, OL.PAYMENT_ID
           ...
           WHERE OL.INSERT_DATE >= TO_DATE(V_FROMDATE, 'YYYYMMDDHH24MISS')
             AND PROC_FLAG NOT IN ('00', '07', '20', '05')
             AND NOT EXISTS
           (SELECT 1
                    FROM TPAORDERM PM
                   WHERE PM.PA_ORDER_GB = '20'
  ...
  */
  ...
  SELECT CASE
    ...
    FROM (SELECT t.pa_code, t.goods_Code
          ...
          SELECT t.pa_code, t.goods_Code
           ...
           WHERE t.pa_code = '91'
             AND EXISTS (SELECT 1
                    FROM tpatmongoods t2, TGOODS TG
                   WHERE t.goods_Code = t2.goods_Code
                     AND T2.GOODS_CODE = TG.GOODS_CODE
                     AND TG.SALE_GB = '00'
                     AND TG.INSERT_DATE > TRUNC(SYSDATE) - 5
                     AND t2.pa_code = '92')
             AND t.goods_code NOT IN ('35123034', '28802074') -- 테스트상품
          UNION
          SELECT t.pa_code, t.goods_Code
            FROM tpassggoods t
           WHERE t.pa_code = 'A1'
  ...
  SELECT CASE
    ...
    FROM (SELECT * 
           ...
           WHERE OD.ORDER_DATE > SYSDATE - 1
             ...
             AND OD.ORDER_NO NOT IN ('20230306559959','20230324139698','20230328665673','20230405444049','20230627356501'
                                    ,'20230822779820'));
  IF (IS_EXISTED > 0) THEN
    INSERT_ERROR_LOG();
  END IF; 
  ...
  SELECT CASE
         ...
         END DATA_CHECK
    INTO IS_EXISTED
    FROM (SELECT DISTINCT OS.ORDER_NO, OS.SHIP_COST_NO as OS_SHIP_COST_NO
