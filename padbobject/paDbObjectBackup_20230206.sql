...
-- Created by hham3 on 2023-02-06, 오전 11:12:18 --
-------------------------------------------------

set define off
spool bmtcomBackup.log
...
prompt =================================
prompt
CREATE OR REPLACE PROCEDURE BMTCOM.sp_tsdcheck_pa(
...
name_varry_P100 v_arry := v_arry('조민하',       '이지영',    '정기복',      '박은진',    '정연진');
tel_varry_P100  v_arry := v_arry('01046355217'  ,'01029352811','01092665720', '01020840414', '01056053913');

name_varry_P200 v_arry := v_arry('조민하',       '이지영',     '정기복',      '박은진',   '정연진');
tel_varry_P200  v_arry := v_arry('01046355217' ,'01029352811','01092665720', '01020840414', '01056053913');

name_varry_P300 v_arry := v_arry('조민하',       '이지영',      '정기복',      '박은진',   '정연진');
tel_varry_P300  v_arry := v_arry('01046355217' ,'01029352811','01092665720', '01020840414', '01056053913');
...
BEGIN
   INSERT INTO TBL_SUBMIT_QUEUE_1 (
         ...
         SELECT
               TO_CHAR(SYSDATE, 'YYYYMMDD') || LTRIM(TO_CHAR(TBL_SUBMIT_QUEUE_1_SEQ.NEXTVAL, '000000'))
              , ''
...
BEGIN
    PKG_COMMON.EXP_PRNS(v_prog_id, '  --  ERROR ' || v_flag || ':' || v_error_msg);
...
-- setting input argument
    ...
    SELECT CASE WHEN XA.HOLIDAY_DAY = '0' AND XA.WORK_YN = '1' THEN
                     '1'
                ELSE
                     '0'
                 END
       INTO v_send_sms_yn
      FROM (
           SELECT CASE WHEN TO_CHAR(d_sysdate , 'D') IN ('2','3','4','5','6') THEN
                            '0'
                       ELSE
                            '1'
                        END HOLIDAY_DAY
    ...
    SELECT NVL(( SELECT '1'
                    FROM DUAL
                    WHERE v_sysdatetime BETWEEN v_sysdate || '081000'
                                            AND v_sysdate || '200000'
               ),'0')
...
--------------------------------------------------------------------------------
    ...
    */
    ...
    v_flag      := 'P106';    v_error_msg := '[확인_P106][제휴_공통] 업체배송지에는 없는 업체담당자 정보가 있는지 확인';
    is_existed  := 0;
    ...
    */
    ...
    v_flag      := 'P301';    v_error_msg := '[즉시_P301][제휴_공통] 판매단계 동기화 점검';
    is_existed  := 0;
    ...
    SELECT CASE WHEN COUNT(1) > 0 THEN 1 ELSE 0 END DATA_CHECK
      into is_existed
      FROM (
            SELECT TG.GOODS_CODE, TG.PA_CODE
              FROM TGOODS GD,
                   TPAGOODSTARGET TG
             WHERE GD.SALE_GB <> '00'
               ...
               AND 1 = CASE WHEN  TG.PA_GROUP_CODE = '01' THEN ( SELECT '1' FROM TPA11STGOODS PG WHERE PG.GOODS_CODE = GD.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' AND PG.TRANS_SALE_YN ='0')
                            WHEN (TG.PA_GROUP_CODE = '02' OR TG.PA_GROUP_CODE = '03' ) THEN ( SELECT '1' FROM TPAGMKTGOODS PG WHERE PG.GOODS_CODE = GD.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' AND PG.TRANS_SALE_YN ='1' AND ROWNUM =1 )
                            WHEN  TG.PA_GROUP_CODE = '04' THEN ( SELECT '1' FROM TPANAVERGOODS PG WHERE PG.GOODS_CODE = GD.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' AND PG.TRANS_SALE_YN ='0')
                            WHEN  TG.PA_GROUP_CODE = '05' THEN ( SELECT '1' FROM TPACOPNGOODS PG WHERE PG.GOODS_CODE = GD.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' AND PG.TRANS_SALE_YN ='0')
                            WHEN  TG.PA_GROUP_CODE = '06' THEN ( SELECT '1' FROM TPAWEMPGOODS PG WHERE PG.GOODS_CODE = GD.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' AND PG.TRANS_SALE_YN ='0')
                            WHEN  TG.PA_GROUP_CODE = '07' THEN ( SELECT '1' FROM TPAINTPGOODS PG WHERE PG.GOODS_CODE = GD.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' AND PG.TRANS_SALE_YN ='0')
                            WHEN  TG.PA_GROUP_CODE = '08' THEN ( SELECT '1' FROM TPALTONGOODS PG WHERE PG.GOODS_CODE = GD.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' AND PG.TRANS_SALE_YN ='0')
                            WHEN  TG.PA_GROUP_CODE = '09' THEN ( SELECT '1' FROM TPATMONGOODS PG WHERE PG.GOODS_CODE = GD.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' AND PG.TRANS_SALE_YN ='0')
                            WHEN  TG.PA_GROUP_CODE = '10' THEN ( SELECT '1' FROM TPASSGGOODS PG WHERE PG.GOODS_CODE = GD.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' AND PG.TRANS_SALE_YN ='0')
                            WHEN  TG.PA_GROUP_CODE = '11' THEN ( SELECT '1' FROM TPAKAKAOGOODS PG WHERE PG.GOODS_CODE = GD.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' AND PG.TRANS_SALE_YN ='0')
                            WHEN  TG.PA_GROUP_CODE = '12' THEN ( SELECT '1' FROM TPAHALFGOODS PG WHERE PG.GOODS_CODE = GD.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' AND PG.TRANS_SALE_YN ='0')
                            ELSE '0' END
           )
           ;
...
,'011053','011038','007527','005423','006679','000284','007509')
               --AND GD.GOODS_CODE NOT IN ('20087466') -> 2019.02.08 THJEON 상품코드에서 브랜드코드로 변경
...
*/
    ...
    v_flag      := 'P306';    v_error_msg := '[확인_P306][제휴_공통] 상품기초정보 동기화 점검';
    is_existed  := 0;
      ...
      FROM (           
            ...
            SELECT GD.GOODS_CODE
              ...
              FROM TPAPRODUCT PG
                 , TGOODS   GD
             WHERE PG.GOODS_CODE = GD.GOODS_CODE
               AND ( PG.MAKECO_CODE <> GD.MAKECO_CODE
                  OR PG.TAX_YN <> GD.TAX_YN
                  OR PG.TAX_SMALL_YN <> GD.TAX_SMALL_YN
                  OR PG.ADULT_YN  <> GD.ADULT_YN
                  OR PG.ORDER_MIN_QTY <> GD.ORDER_MIN_QTY
                  OR PG.ORDER_MAX_QTY <> GD.ORDER_MAX_QTY
                  OR PG.CUST_ORD_QTY_CHECK_TERM <> GD.CUST_ORD_QTY_CHECK_TERM
                  OR PG.ENTP_CODE <> GD.ENTP_CODE
                  OR PG.SHIP_MAN_SEQ <> GD.SHIP_MAN_SEQ
                  OR PG.RETURN_MAN_SEQ <> GD.RETURN_MAN_SEQ
                  OR PG.SHIP_COST_CODE <> GD.SHIP_COST_CODE
                  OR PG.AVG_DELY_LEADTIME <> GD.AVG_DELY_LEADTIME
                  OR PG.COLLECT_YN <> GD.COLLECT_YN
                   )
               AND GD.MODIFY_DATE >= d_sysdate-30
               AND GD.MODIFY_DATE < SYSDATE - INTERVAL '60' MINUTE
               AND GD.SALE_GB <> '19'
               AND ( PG.SALE_PA_CODE LIKE '5%' OR PG.SALE_PA_CODE LIKE 'B%' OR PG.SALE_PA_CODE LIKE 'C%' OR PG.SALE_PA_CODE LIKE '1%' OR PG.SALE_PA_CODE LIKE '2%' OR PG.SALE_PA_CODE LIKE '4%'                
               OR PG.SALE_PA_CODE LIKE '6%' OR PG.SALE_PA_CODE LIKE '7%' OR PG.SALE_PA_CODE LIKE '8%' OR PG.SALE_PA_CODE LIKE '9%' OR PG.SALE_PA_CODE LIKE 'A%' ) 
               -- 조건에 맞지 않을 경우 TPAPRODUCT업데이트 하지 않고 판매중단 시키기 때문에 제휴사 판매중인 상품만 검증 처리 2022.04.25 by jchoi
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE 'B%' THEN NVL(( SELECT 1 FROM TPAKAKAOGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE 'C%' THEN NVL(( SELECT 1 FROM TPAHALFGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE 'A%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END 
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '9%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END    
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '8%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '7%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '6%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '5%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END                         
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '4%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '2%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN PG.SALE_PA_CODE LIKE '1%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PG.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END  
               AND GD.GOODS_CODE NOT IN ('23976603')
    ...
    v_flag      := 'P313';    v_error_msg := '[즉시_P313][제휴_공통] 판매제휴사 데이터 점검';
    is_existed  := 0;
      ...
      FROM (
            SELECT G.GOODS_CODE
             ...
             WHERE G.MODIFY_DATE > SYSDATE - 1
               ...
               AND ( EXISTS ( SELECT 1 FROM TPA11STGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE <> DECODE(G.SOURCING_MEDIA,'01','11','61','12','99') )
                  OR EXISTS ( SELECT 1 FROM TPAGMKTGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE <> DECODE(G.SOURCING_MEDIA,'01','21','61','22','99') )
                  --NAVER는 무조건 41
                  OR EXISTS ( SELECT 1 FROM TPACOPNGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE <> DECODE(G.SOURCING_MEDIA,'01','51','61','52','99') )
                  OR EXISTS ( SELECT 1 FROM TPAWEMPGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE <> DECODE(G.SOURCING_MEDIA,'01','61','61','62','99') )
                  OR EXISTS ( SELECT 1 FROM TPAINTPGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE <> DECODE(G.SOURCING_MEDIA,'01','71','61','72','99') )
                  OR EXISTS ( SELECT 1 FROM TPALTONGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE <> DECODE(G.SOURCING_MEDIA,'01','81','61','82','99') )
                  ...
                  OR EXISTS ( SELECT 1 FROM TPASSGGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE <> DECODE(G.SOURCING_MEDIA,'01','A1','61','A2','99')  )
                  OR EXISTS ( SELECT 1 FROM TPAKAKAOGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE <> DECODE(G.SOURCING_MEDIA,'01','B1','61','B2','99'))
                  OR EXISTS ( SELECT 1 FROM TPAHALFGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE <> DECODE(G.SOURCING_MEDIA,'01','C1','61','C2','99'))

                   )
               AND G.GOODS_CODE NOT IN ('25677110','25677126','25677213','25679058','25679101','25679139','25679178','25679186','25697949','25697958',
                                        '25697959','25697966','25697975','25738045','25738063','25738085','25738127','25797745','25679068','25738051',
                                        '25679112','20007357','25679176','20001439','20001552','20007446','20007447','20007448','20007449','25677133',
                                        '25738001', '20000392', '20000985', '20001245', '20001268', '20001279', '20089304', '20174729', '25839729','20021480', '25839728','20007440','20007442'
                                        ,'24310357'
             ...
             SELECT G.GOODS_CODE
              ...
              WHERE G.MODIFY_DATE > SYSDATE - 1
                ...
                AND ( EXISTS ( SELECT 1 FROM TPA11STGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE = '11' )
                   OR EXISTS ( SELECT 1 FROM TPAGMKTGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE = '21' )
                   OR EXISTS ( SELECT 1 FROM TPACOPNGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE = '51' )
                   OR EXISTS ( SELECT 1 FROM TPAWEMPGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE = '61' )
                   OR EXISTS ( SELECT 1 FROM TPAINTPGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE = '71' )
                   OR EXISTS ( SELECT 1 FROM TPALTONGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE = '81' )
                   OR EXISTS ( SELECT 1 FROM TPATMONGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE = '91' )
                   OR EXISTS ( SELECT 1 FROM TPASSGGOODS  PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE = 'A1' )
                   OR EXISTS ( SELECT 1 FROM TPAKAKAOGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE = 'B1' )
                   OR EXISTS ( SELECT 1 FROM TPAHALFGOODS PG WHERE PG.GOODS_CODE = G.GOODS_CODE AND PG.PA_CODE = 'C1' )
                   )
           )
           ;
    ...
    SELECT CASE WHEN COUNT(1) > 0 THEN 1 ELSE 0 END DATA_CHECK
      ...
      FROM (  
              ...
              SELECT DT.GOODS_CODE, DT.GOODSDT_CODE
               ...
               WHERE DT.MODIFY_DATE > d_sysdate - 30
                 ...
                 AND 1 = CASE WHEN PP.SALE_PA_CODE LIKE '1%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PP.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
                 AND 1 = CASE WHEN PP.SALE_PA_CODE LIKE '2%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PP.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
                 AND 1 = CASE WHEN PP.SALE_PA_CODE LIKE '4%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PP.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
                 AND 1 = CASE WHEN PP.SALE_PA_CODE LIKE '5%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PP.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
                 AND 1 = CASE WHEN PP.SALE_PA_CODE LIKE '6%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PP.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
                 AND 1 = CASE WHEN PP.SALE_PA_CODE LIKE '7%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PP.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
                 AND 1 = CASE WHEN PP.SALE_PA_CODE LIKE '8%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PP.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
                 AND 1 = CASE WHEN PP.SALE_PA_CODE LIKE '9%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PP.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
                AND 1 = CASE WHEN PP.SALE_PA_CODE LIKE 'A%' THEN NVL(( SELECT 1 FROM TPACOPNGOODS TPG WHERE TPG.GOODS_CODE = PP.GOODS_CODE AND TPG.PA_SALE_GB = '20' AND TPG.PA_STATUS = '30'),0)
                            ELSE 1 END
    ...
    v_flag      := 'P315';    v_error_msg := '[즉시_P315][제휴_공통] 단품코드에는 존재하나 공통 상품-단품 매핑 테이블에 존재하지 않는건';
    is_existed  := 0;
                ...
                /*신규 동기화용*/
                 SELECT DISTINCT GD.GOODS_CODE ,
                           ...
                           WHERE GD.MODIFY_DATE >= d_sysdate - 10
                             AND GD.GOODS_CODE = TG.GOODS_CODE
                             AND TG.PA_GROUP_CODE IN ('01','02','04','05','06','07','09','11')
                             AND GD.SALE_GB != '19'
                             AND EXISTS ( SELECT 'X'
                            ...
                            AND (
                                	 NOT EXISTS ( SELECT 1 FROM TPAGOODSDTMAPPING MP WHERE MP.GOODS_CODE = GD.GOODS_CODE AND MP.GOODSDT_CODE = GD.GOODSDT_CODE AND MP.PA_CODE = TG.PA_CODE )
                                    AND  1 = ( CASE 
                                                         WHEN TG.PA_GROUP_CODE = '01' THEN (SELECT 1 FROM TPA11STGOODS PG WHERE PG.GOODS_CODE = TG.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' )
                                                         WHEN TG.PA_GROUP_CODE = '02' THEN (SELECT 1 FROM TPAGMKTGOODS PG WHERE PG.GOODS_CODE = TG.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' )
                                                         WHEN TG.PA_GROUP_CODE = '04' THEN (SELECT 1 FROM TPANAVERGOODS PG WHERE PG.GOODS_CODE = TG.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' )
                                                         WHEN TG.PA_GROUP_CODE = '05' THEN (SELECT 1 FROM TPACOPNGOODS PG WHERE PG.GOODS_CODE = TG.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' )                                        
                                                         WHEN TG.PA_GROUP_CODE = '06' THEN (SELECT 1 FROM TPAWEMPGOODS PG WHERE PG.GOODS_CODE = TG.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' )
                                                         WHEN TG.PA_GROUP_CODE = '07' THEN (SELECT 1 FROM TPAINTPGOODS PG WHERE PG.GOODS_CODE = TG.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' )               
                                                         WHEN TG.PA_GROUP_CODE = '09' THEN (SELECT 1 FROM TPATMONGOODS PG WHERE PG.GOODS_CODE = TG.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' )   
                                                         WHEN TG.PA_GROUP_CODE = '11' THEN (SELECT 1 FROM TPAKAKAOGOODS PG WHERE PG.GOODS_CODE = TG.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' )
                                                         WHEN TG.PA_GROUP_CODE = '12' THEN (SELECT 1 FROM TPAHALFGOODS PG WHERE PG.GOODS_CODE = TG.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30' )
                                                         END )
                                )
           )
           ;
    ...
    -- END --
*/
    v_flag      := 'P317';    v_error_msg := '[확인_P317][제휴_공통] 정보고시가 없는건 점검 ( TGOODS <-> TPAGOODSOFFER )';
    is_existed  := 0;
 ...
 /* 신규 가격 테이블로(TPAGOODSPRICEAPPLY) 변경되어 주석처리  P329로 대체
   ...
   */ 
    ...
    */
    ...
    v_flag      := 'P324';    v_error_msg := '[확인_P324][제휴_11번가] 상품정보 기초코드 데이터 유효성 검사';
    is_existed  := 0;
    ...
    v_flag      := 'P325';    v_error_msg := '[확인_P325][제휴_공통] 제휴상품 테이블에는 존재하는 상품코드이나 제휴공통 상품코드에 존재하지 않은 건';
    is_existed  := 0;
    ...
    v_flag      := 'P326';    v_error_msg := '[확인_P326][제휴_공통] 수정일시가 동기화 배치 시간을 지났는데 전송대상여부가 1 인건';
    is_existed  := 0;
      ...
      FROM (
            SELECT PA11.GOODS_CODE
             ...
             WHERE PA11.TRANS_TARGET_YN ='1'
               ...
               AND PA11.MODIFY_DATE < SYSDATE - INTERVAL '60' MINUTE
               AND NOT EXISTS ( SELECT '1'
                                  FROM TGOODS GD
            ...
            SELECT PAGM.GOODS_CODE
             ...
             WHERE PAGM.TRANS_TARGET_YN ='1'
               ...
               AND PAGM.MODIFY_DATE < SYSDATE - INTERVAL '60' MINUTE
            ...
            SELECT COPN.GOODS_CODE
             ...
             WHERE COPN.PA_CODE IN ('51','52')
               ...
               AND COPN.MODIFY_DATE < SYSDATE - INTERVAL '60' MINUTE
               --위부분 추가 , 지마켓 전송시간 3분,33분이기에 최근 변경60분 이내데이터는 검색하지않음
               AND NOT EXISTS ( SELECT '1'
                                  FROM TGOODS GD
            ...
            SELECT WEMP.GOODS_CODE
             ...
             WHERE WEMP.PA_CODE IN ('61','62')
               ...
               AND WEMP.MODIFY_DATE < SYSDATE - INTERVAL '60' MINUTE
               --위부분 추가 , 지마켓 전송시간 3분,33분이기에 최근 변경60분 이내데이터는 검색하지않음
               AND NOT EXISTS ( SELECT '1'
                                  FROM TGOODS GD
            ...
            SELECT PG.GOODS_CODE
             ...
             WHERE PG.TRANS_TARGET_YN ='1'
               ...
               --아래부분 추가 THJEON
               --AND PG.PA_SALE_GB = '20'
               AND PG.MODIFY_DATE < SYSDATE - INTERVAL '60' MINUTE
               --위부분 추가 , 지마켓 전송시간 3분,33분이기에 최근 변경60분 이내데이터는 검색하지않음
               AND NOT EXISTS ( SELECT '1'
                                  FROM TGOODS GD
                                 WHERE PG.GOODS_CODE = GD.GOODS_CODE
            ...
            SELECT LTON.GOODS_CODE
             ...
             WHERE LTON.TRANS_TARGET_YN ='1'
               ...
               AND LTON.MODIFY_DATE < SYSDATE - INTERVAL '60' MINUTE
               AND NOT EXISTS ( SELECT '1'
                                  FROM TGOODS GD
            ...
            SELECT TMON.GOODS_CODE
             ...
             WHERE TMON.TRANS_TARGET_YN ='1'
               ...
               AND TMON.MODIFY_DATE < SYSDATE - INTERVAL '60' MINUTE
               AND NOT EXISTS ( SELECT '1'
                                  FROM TGOODS GD
            ...
            SELECT SSG.GOODS_CODE
             ...
             WHERE SSG.TRANS_TARGET_YN ='1'
               ...
               AND SSG.MODIFY_DATE < SYSDATE - INTERVAL '60' MINUTE
               AND NOT EXISTS ( SELECT '1'
                                  FROM TGOODS GD
            ...
            SELECT KG.GOODS_CODE
             ...
             WHERE KG.TRANS_TARGET_YN ='1'
               ...
               AND KG.MODIFY_DATE < SYSDATE - INTERVAL '60' MINUTE
               AND NOT EXISTS ( SELECT '1'
                                  FROM TGOODS GD
            ...
            SELECT HG.GOODS_CODE
             ...
             WHERE HG.TRANS_TARGET_YN ='1'
               ...
               AND HG.MODIFY_DATE < SYSDATE - INTERVAL '60' MINUTE
               AND NOT EXISTS ( SELECT '1'
                                  FROM TGOODS GD
    ...
    -- END --
     ...
     v_flag      := 'P332';    v_error_msg := '[즉시_P332][제휴_공통]출고지별 배송비 미전송건';
     is_existed  := 0;

    /* P332. [제휴_공통]출고지별 배송비 미전송건*/
    SELECT CASE WHEN COUNT(1) > 0 THEN 1 ELSE 0 END DATA_CHECK
      into is_existed
      FROM (SELECT PCC.ENTP_CODE FROM TPACUSTCNSHIPCOST PCC
             WHERE PCC.TRANS_CN_COST_YN = '1'
               ...
               AND ENTP_CODE NOT IN ('104529','105032','108140','108849','112182')
                       );

      IF( is_existed > 0 ) THEN insert_error_log(); END IF;
    -- END --

    v_flag      := 'P333';    v_error_msg := '[즉시_P333][제휴_공통]출고지별 배송비 동기화미처리건';
    is_existed  := 0;
    ...
    SELECT CASE WHEN COUNT(1) > 0 THEN 1 ELSE 0 END DATA_CHECK
      ...
      FROM (SELECT 1
             ...
             WHERE PCS.SHIP_COST_CODE LIKE 'CN%'
               ...
               AND EXISTS (  -- YEKIM 19/10/18, TPACUSTSHIPCOST TB는 11번가만 사용. 11번가 입점 된 상품들 한해 SD체크
                   SELECT 1 FROM TPAGOODS PG,
                                 TPA11STGOODS PGG
    ...
    v_flag      := 'P327';    v_error_msg := '[확인_P327][제휴_공통] 제휴상품코드가 존재하는데 입점상태가 입점완료가 아닌건';
    is_existed  := 0;
      ...
      FROM (
            SELECT PA11.GOODS_CODE
             ...
             WHERE PA11.PRODUCT_NO IS NOT NULL
               ...
               AND PA11.PA_SALE_GB ='20'
               AND PA11.MODIFY_DATE >= d_sysdate-30
               AND PA11.GOODS_CODE NOT IN ('20033069','20033070','20033072','20033680','20033830')
            ...
            SELECT PAGM.GOODS_CODE
             ...
             WHERE PAGM.ITEM_NO IS NOT NULL
               ...
               AND PAGM.PA_SALE_GB ='20'
               AND PAGM.MODIFY_DATE >= d_sysdate-30
            ...
            SELECT PACP.GOODS_CODE
              ...
              FROM TPACOPNGOODS PACP
             WHERE PACP.SELLER_PRODUCT_ID IS NOT NULL
               AND PACP.PA_STATUS NOT IN('30','40','70','90','80') --90은 수기로 상품 연동끊은 건. 80은 분리요청, 70은 모지???
               AND PACP.PA_SALE_GB ='20'
               AND PACP.MODIFY_DATE >= d_sysdate-30
            ...
            SELECT WEMP.GOODS_CODE
              ...
              FROM TPAWEMPGOODS WEMP
             WHERE WEMP.PRODUCT_NO IS NOT NULL
               AND WEMP.PA_STATUS NOT IN('30','40','90') --90은 수기로 상품 연동끊은 건.
               AND WEMP.PA_SALE_GB ='20'
               AND WEMP.MODIFY_DATE >= d_sysdate-30
            ...
            SELECT PG.GOODS_CODE
             ...
             WHERE PG.PRD_NO IS NOT NULL
               AND PG.PA_STATUS NOT IN('30','40','90') --90은 수기로 상품 연동끊은 건.
               AND PG.PA_SALE_GB ='20'
               AND PG.MODIFY_DATE >= d_sysdate-30
            ...
            SELECT LTON.GOODS_CODE
             ...
             WHERE LTON.SPD_NO IS NOT NULL
               ...
               AND LTON.PA_SALE_GB ='20'
               AND LTON.MODIFY_DATE >= d_sysdate-30
            ...
            SELECT TMON.GOODS_CODE
             ...
             WHERE TMON.DEAL_NO IS NOT NULL
               ...
               AND TMON.PA_SALE_GB ='20'
               AND TMON.MODIFY_DATE >= d_sysdate-30
            ...
            SELECT SSG.GOODS_CODE
             ...
             WHERE SSG.ITEM_ID IS NOT NULL
               ...
               AND SSG.PA_SALE_GB ='20'
               AND SSG.MODIFY_DATE >= d_sysdate-30
            ...
            SELECT KG.GOODS_CODE
             ...
             WHERE KG.PRODUCT_ID IS NOT NULL
               ...
               AND KG.PA_SALE_GB ='20'
               AND KG.MODIFY_DATE >= d_sysdate-30
            ...
            SELECT HG.GOODS_CODE
             ...
             WHERE HG.PRODUCT_NO IS NOT NULL
               ...
               AND HG.PA_SALE_GB ='20'
               AND HG.MODIFY_DATE >= d_sysdate-30
    ...
    v_flag      := 'P328';    v_error_msg := '[확인_P328][제휴_G마켓] 상품정보 기초코드 데이터 유효성 검사';
    is_existed  := 0;
    ...
    SELECT CASE WHEN COUNT(1) > 0 THEN 1 ELSE 0 END DATA_CHECK
      ...
      FROM (
             ...
             SELECT PAGK.GOODS_CODE
               FROM TPAGMKTGOODS PAGK
              WHERE PAGK.ORIGIN_ENUM NOT IN ( SELECT TC.CODE_MGROUP FROM TCODE TC WHERE TC.CODE_LGROUP = 'O564' AND TC.USE_YN = 1)
                AND PAGK.MODIFY_DATE >= d_sysdate-5
    ...
    v_flag      := 'P329';    v_error_msg := '[즉시_P329][제휴_공통] 가격/옵션 정보 전송 안된건';
    is_existed  := 0;
    ...
    SELECT CASE WHEN COUNT(1) > 0 THEN 1 ELSE 0 END DATA_CHECK
      into is_existed
      FROM (
              SELECT GPA.GOODS_CODE
                   , GPA.PA_CODE
                   , '1'
                FROM TPAGOODSPRICEAPPLY GPA
               WHERE GPA.TRANS_DATE IS NULL
                 AND GPA.INSERT_DATE >= d_sysdate - 3
                 AND GPA.INSERT_DATE < SYSDATE - INTERVAL '60' MINUTE
                 AND ( GPA.APPLY_DATE, GPA.PRICE_APPLY_SEQ ) IN ( SELECT /*+ INDEX_DESC (PP PK_TPAGOODSPRICEAPPLY)*/
                                                                  ...
                                                                  WHERE PP.GOODS_CODE = GPA.GOODS_CODE
                                                                    ...
                                                                    AND PP.APPLY_DATE < SYSDATE
                                                                     AND ROWNUM = 1 )
                  AND 1 = CASE WHEN GPA.PA_CODE IN ('B1','B2') THEN ( SELECT 1 FROM TPAKAKAOGOODS PG WHERE PG.GOODS_CODE = GPA.GOODS_CODE AND PG.PA_CODE = GPA.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20')
                               WHEN GPA.PA_CODE IN ('C1','C2') THEN ( SELECT 1 FROM TPAHALFGOODS PG WHERE PG.GOODS_CODE = GPA.GOODS_CODE AND PG.PA_CODE = GPA.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20')
                               WHEN GPA.PA_CODE IN ('21','22') THEN ( SELECT 1 FROM TPAGMKTGOODS PG WHERE PG.GOODS_CODE = GPA.GOODS_CODE AND PG.PA_CODE = GPA.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' AND PG.PA_GROUP_CODE = GPA.PA_GROUP_CODE)
                           WHEN GPA.PA_CODE IN ('11','12') THEN ( SELECT 1 FROM TPA11STGOODS PG WHERE PG.GOODS_CODE = GPA.GOODS_CODE AND PG.PA_CODE = GPA.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' )
                            WHEN GPA.PA_CODE IN ('41') THEN ( SELECT 1 FROM TPANAVERGOODS PG WHERE PG.GOODS_CODE = GPA.GOODS_CODE AND PG.PA_CODE = GPA.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' )
                            WHEN GPA.PA_CODE IN ('51','52') THEN ( SELECT 1 FROM TPACOPNGOODS PG WHERE PG.GOODS_CODE = GPA.GOODS_CODE AND PG.PA_CODE = GPA.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' AND PG.APPROVAL_STATUS = '30')
                            WHEN GPA.PA_CODE IN ('61','62') THEN ( SELECT 1 FROM TPAWEMPGOODS PG WHERE PG.GOODS_CODE = GPA.GOODS_CODE AND PG.PA_CODE = GPA.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' )
                            WHEN GPA.PA_CODE IN ('71','72') THEN ( SELECT 1 FROM TPAINTPGOODS PG WHERE PG.GOODS_CODE = GPA.GOODS_CODE AND PG.PA_CODE = GPA.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' )
                            WHEN GPA.PA_CODE IN ('81','82') THEN ( SELECT 1 FROM TPALTONGOODS PG WHERE PG.GOODS_CODE = GPA.GOODS_CODE AND PG.PA_CODE = GPA.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' )
                            WHEN GPA.PA_CODE IN ('91','92') THEN ( SELECT 1 FROM TPATMONGOODS PG WHERE PG.GOODS_CODE = GPA.GOODS_CODE AND PG.PA_CODE = GPA.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' )
                            WHEN GPA.PA_CODE IN ('A1','A2') THEN ( SELECT 1 FROM TPASSGGOODS PG WHERE PG.GOODS_CODE = GPA.GOODS_CODE AND PG.PA_CODE = GPA.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' AND PG.PROC_STAT_CD = '20' )

                       ELSE 0 END

             UNION ALL

            SELECT distinct M.GOODS_CODE
                 , M.PA_CODE
                 , '2'
              FROM TPAGOODSDTMAPPING M
             WHERE M.TRANS_TARGET_YN = '1'
               AND M.MODIFY_DATE >= d_sysdate-3
               AND M.MODIFY_DATE < SYSDATE - INTERVAL '60' MINUTE
               AND  1 = CASE WHEN M.PA_CODE IN ('11','12') THEN (SELECT 1 FROM TPA11STGOODS PG WHERE PG.GOODS_CODE = M.GOODS_CODE  AND PG.PA_CODE = M.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' AND PG.MODIFY_DATE > SYSDATE - INTERVAL '60' MINUTE )
                             WHEN M.PA_CODE IN ('21','22') THEN (SELECT 1 FROM TPAGMKTGOODS PG WHERE PG.GOODS_CODE = M.GOODS_CODE  AND PG.PA_CODE = M.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' AND rownum = 1 )
                             WHEN M.PA_CODE IN ('51','52') THEN (SELECT 1 FROM TPACOPNGOODS PG WHERE PG.GOODS_CODE = M.GOODS_CODE  AND PG.PA_CODE = M.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' AND PG.MODIFY_DATE > SYSDATE - INTERVAL '60' MINUTE AND PG.APPROVAL_STATUS = '30')
                             WHEN M.PA_CODE IN ('61','62') THEN (SELECT 1 FROM TPAWEMPGOODS PG WHERE PG.GOODS_CODE = M.GOODS_CODE  AND PG.PA_CODE = M.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' AND PG.MODIFY_DATE > SYSDATE - INTERVAL '60' MINUTE )
                             WHEN M.PA_CODE IN ('71','72') THEN (SELECT 1 FROM TPAINTPGOODS PG WHERE PG.GOODS_CODE = M.GOODS_CODE  AND PG.PA_CODE = M.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' AND PG.MODIFY_DATE > SYSDATE - INTERVAL '60' MINUTE )
                             WHEN M.PA_CODE IN ('91','92') THEN (SELECT 1 FROM TPATMONGOODS PG WHERE PG.GOODS_CODE = M.GOODS_CODE  AND PG.PA_CODE = M.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' AND PG.MODIFY_DATE > SYSDATE - INTERVAL '60' MINUTE )
                             WHEN M.PA_CODE IN ('B1','B2') THEN (SELECT 1 FROM TPAKAKAOGOODS PG WHERE PG.GOODS_CODE = M.GOODS_CODE  AND PG.PA_CODE = M.PA_CODE AND PG.PA_STATUS = '30' AND PG.PA_SALE_GB = '20' AND PG.MODIFY_DATE > SYSDATE - INTERVAL '60' MINUTE )
                              ELSE 0 END
             AND M.TRANS_ORDER_ABLE_QTY  != 0

             UNION ALL

            SELECT DM.GOODS_CODE
                 , DM.PA_CODE
                 , '2'
              FROM TPALTONGOODS PG
    ...
    v_flag      := 'P336';  v_error_msg := '[즉시_P336] 11번가 제휴사단품KEY(옵션코드) 중복 검증';
    is_existed  := 0;
...
/*    v_flag      := 'P338';  v_error_msg := '[즉시_P338] 네이버 제휴사단품KEY 중복 검증';
    ...
    v_flag      := 'P335';    v_error_msg := '[확인_P335]상품고시정보 필수 데이터 없는 경우';
    is_existed  := 0;

    /* P335. 상품고시정보 필수 데이터 없는 경우  여기다가 STATUS 빼면 ..*/
    SELECT CASE WHEN COUNT(1) > 0 THEN 1 ELSE 0 END DATA_CHECK
      into is_existed
      FROM ( SELECT TPG.GOODS_CODE
                 , TPG.PA_CODE
                 , TPG.PA_GROUP_CODE
              FROM TGOODS GD
                 , TPAGOODSTARGET TPG
             WHERE GD.SALE_GB = '00'
               ...
               AND NOT EXISTS ( SELECT 1
                                 ...
                                 WHERE PO.GOODS_CODE = TPG.GOODS_CODE
                                   AND PO.PA_GROUP_CODE = DECODE(TPG.PA_GROUP_CODE,'03','02',TPG.PA_GROUP_CODE)
                                   AND PO.USE_YN = '1')
               AND 1 = CASE WHEN TPG.PA_GROUP_CODE = '01' THEN NVL((SELECT 1 FROM TPA11STGOODS PG WHERE PG.PA_GROUP_CODE = TPG.PA_GROUP_CODE AND PG.PA_CODE = TPG.PA_CODE AND PG.GOODS_CODE = TPG.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN TPG.PA_GROUP_CODE IN ('02','03') THEN NVL((SELECT 1 FROM TPAGMKTGOODS PG WHERE PG.PA_GROUP_CODE = TPG.PA_GROUP_CODE AND PG.PA_CODE = TPG.PA_CODE AND PG.GOODS_CODE = TPG.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30'),0)
               ...
               AND 1 = CASE WHEN TPG.PA_GROUP_CODE = '11' THEN NVL((SELECT 1 FROM TPAKAKAOGOODS PG WHERE PG.PA_GROUP_CODE = TPG.PA_GROUP_CODE AND PG.PA_CODE = TPG.PA_CODE AND PG.GOODS_CODE = TPG.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30'),0)
                            ELSE 1 END
               AND 1 = CASE WHEN TPG.PA_GROUP_CODE = '12' THEN NVL((SELECT 1 FROM TPAHALFGOODS PG WHERE PG.PA_GROUP_CODE = TPG.PA_GROUP_CODE AND PG.PA_CODE = TPG.PA_CODE AND PG.GOODS_CODE = TPG.GOODS_CODE AND PG.PA_SALE_GB = '20' AND PG.PA_STATUS = '30'),0)
                            ELSE 1 END
           )
    ;
    ...
    v_flag      := 'P339';    v_error_msg := '[확인_P339][제휴_네이버] 전일자 기준으로 정산데이터가 적재 되었는지 확인';
    is_existed  := 0;
...
*/
    ...
    v_flag      := 'P341';    v_error_msg := '[확인_P341][제휴_네이버] 네이버 주소지 정보에서 주소명이 중복된 건';
    is_existed  := 0;
    ...
    SELECT CASE WHEN COUNT(1) > 0 THEN 1 ELSE 0 END DATA_CHECK
      ...
      WHERE NAME NOT IN ('107820004', '101021026', '101021023', '107820005', '208818002', '108365005', '109214014', '109214015', '109214003', '109214004');

    IF( is_existed > 0 ) THEN insert_error_log(); END IF;

    v_flag      := 'P343';    v_error_msg := '[확인_P343][제휴_쿠팡] 구매옵션 미 생성건';
    is_existed  := 0;
    ...
    v_flag      := 'P344';    v_error_msg := '[즉시_P344][제휴_카카오] 상품가격정보 동기화 점검';
    is_existed  := 0;
    ...
    SELECT CASE WHEN COUNT(1) > 0 THEN 1 ELSE 0 END DATA_CHECK
      into is_existed
      FROM (
        SELECT AA.GOODS_CODE  FROM
          ( SELECT DISTINCT BP.PA_CODE, BP.GOODS_CODE , GP.APPLY_DATE FROM
              (
              
             SELECT  CG.PA_CODE
                    , CG.GOODS_CODE
                FROM  TPA11STGOODS CG
               WHERE CG.MODIFY_DATE > sysdate - 30
              ...
              SELECT  KG.PA_CODE
                    , KG.GOODS_CODE
                FROM  TPAKAKAOGOODS KG
               WHERE KG.MODIFY_DATE > d_sysdate - 30
              ...
              SELECT  HG.PA_CODE
                    , HG.GOODS_CODE
                FROM  TPAHALFGOODS HG
               WHERE HG.MODIFY_DATE > d_sysdate - 30
              ...
              WHERE GP.GOODS_CODE = BP.GOODS_CODE
               ...
               AND GP.GOODS_CODE NOT IN ('26922336','27948024')
               ) AA
    ...
    v_flag      := 'P345';    v_error_msg := '[확인_P345][제휴_하프클럽] 브랜드 교환환불불가 속성 불일치건 검출';
    is_existed  := 0;
    ...
    v_flag      := 'P346';    v_error_msg := '[확인_P346][제휴_하프클럽] 배송템플릿 점검';
    is_existed  := 0;
  ...
  SELECT CASE WHEN COUNT(1) > 0 THEN 1 ELSE 0 END DATA_CHECK
    ...
    FROM ( SELECT HG.PA_CODE,
     ...
     v_flag      := 'P347';    v_error_msg := '[확인_P347][제휴_하프클럽] 당사배송상품 검출';
     is_existed  := 0;
    ...
    v_flag      := 'P348';    v_error_msg := '[확인_P348][제휴_하프클럽] 배송템플릿 출고지구분값30 또는 회수지구분값20이 아닌 건';
    is_existed  := 0;
    ...
    v_flag      := 'P349';    v_error_msg := '[즉시_P349][제휴_하프클럽] 하프클럽 배치 3시간 이상 멈춤 점검';
    is_existed  := 0;
    ...
    v_flag      := 'P350';    v_error_msg := '[확인_P350][제휴_하프클럽] 기준정보등록 실패건';
    is_existed  := 0;
      ...
      FROM (
        SELECT HG.GOODS_CODE
         ...
         WHERE HG.RETURN_NOTE LIKE '%IF_PAHALFAPI_01_002%'
           AND HG.RETURN_NOTE NOT LIKE '%표준아이템을 3depth로 정확하게 설정해주시기 바랍니다.%'
    ...
    v_flag      := 'P351';    v_error_msg := '[즉시_P351][제휴_하프클럽] 업체의 출고지/회수지 주소와 배송템플릿의 출고지/회수지 주소가 다른건';
    is_existed  := 0;
    ...
    SELECT CASE WHEN COUNT(1) > 0 THEN 1 ELSE 0 END DATA_CHECK
      into is_existed
      FROM (
        SELECT DECODE (IE.ROAD_ADDR_YN,'1', IE.ROAD_ADDR, IE.ADDR)
          FROM TPAHALFSHIPINFOHISTORY A
             , TENTPUSER OE
             , TENTPUSER IE
         WHERE (A.PA_SHIP_COST_CODE, A.SEQ)  = (SELECT B.PA_SHIP_COST_CODE , MAX(B.SEQ)
          ...
          --출고
           ...
           AND (OE.POST_NO   <> A.POST_NO
            OR  IE.POST_NO   <> A.RET_POST_NO
            OR  NVL(OE.ROAD_POST_ADDR, OE.POST_ADDR) <> A.ADDR
            OR  NVL(IE.ROAD_POST_ADDR, IE.POST_ADDR) <> A.RET_ADDR
            OR  DECODE (OE.ROAD_ADDR_YN,'1', OE.ROAD_ADDR, OE.ADDR) <> A.ADDR_DT
            OR  DECODE (IE.ROAD_ADDR_YN,'1', IE.ROAD_ADDR, IE.ADDR) <> A.RET_ADDR_DT)
           )
    ;
    IF( is_existed > 0 ) THEN insert_error_log(); END IF;
    
    v_flag      := 'P352';    v_error_msg := '[참고_P352][제휴_하프클럽] 하프클럽 브랜드 매핑 변경된 상품';
    is_existed  := 0;
    ...
    v_flag      := 'P353';    v_error_msg := '[즉시_P353][제휴_하프클럽] 브랜드가 중복으로 들어간 건';
    is_existed  := 0;        
    /* P353. [제휴_하프클럽] 브랜드가 중복으로 들어간 건 */
    SELECT CASE WHEN COUNT(1) > 0 THEN 1 ELSE 0 END DATA_CHECK
      into is_existed
      FROM (
            SELECT PB.PA_BRAND_NAME
             , PB.PA_CODE 
    ...
    v_flag      := 'P354';    v_error_msg := '[확인_P354][제휴_하프클럽] 제주/도서산간배송불가 배송템플릿 점검';
    is_existed  := 0;
  ...
  /*P354. [제휴_하프클럽] 제주/도서산간배송불가 배송템플릿 점검, 검출시 TPAHALFGOODS.TRANS_TARGET_YN='1'로 UPDATE*/
    ...
    v_flag      := 'P399';   v_error_msg := '[알림_P399]제휴 데이터 정합성 실행 완료';
    SELECT v_error_msg
    into v_error_msg FROM DUAL;
    insert_error_log();
...
-- execution time  ------------------------
    ...
    PKG_COMMON.EXP_PRNS(v_prog_id, '');
        ...
        SELECT 'Execution Time = ' ||
               FLOOR((SYSDATE - TO_DATE(v_sysdatetime, 'YYYYMMDDHH24MISS')) * 24 * 60 ) || ' min  ' ||
               FLOOR(MOD(((SYSDATE - TO_DATE(v_sysdatetime, 'YYYYMMDDHH24MISS')) * 24 * 60 * 60), 60 )) || ' second'
          INTO v_msg
          FROM dual ;
    ...
    PKG_COMMON.HISTORY_SUCCESS(v_prog_id, v_user_id, TO_CHAR((SYSDATE - 1), 'YYYYMMDD') , '', v_msg);
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        UTL_FILE.FCLOSE_ALL;
        PKG_COMMON.HISTORY_FAIL(v_prog_id, v_user_id, TO_CHAR((SYSDATE - 1), 'YYYYMMDD'), '', trim(v_error_msg) || trim(v_db_error_msg));
        --raise;
...
BEGIN
  ...
  SELECT CASE
    ...
    FROM (SELECT /*+USE_HASH(PO)*/
           ...
           HOUR
             AND PO.PA_ORDER_NO NOT IN ('202001028142447',
                                        '31000081775927' -- 9/28
                                       ,
                                        '23000081802233' -- 9/28 부분 회수확정
                                       ,
                                        '159067041' -- 10/5 주문상태 상이, 문의중
                                       ,
                                        '1276156026' --10/15 주문상태 상
                                       ,
                                        '2020093048732061' --10/16 네이버주문
                                       ,
                                        '21000084521318' -- 10/27 쿠팡 부분처리
                                       ,
                                        '1372920942' -- 1/14 주문단계 상이
                                       ,
                                        '14000093579230' -- 2/10 주문단계 상이
                                       ,
                                        '28000096072268' --3/313 쿠팡 주문단계 상이
                                       ,
                                        '2021111174840421' -- 12/7 네이버 주문상태 상
                                       ,
                                        '3097461150' -- 1/4 티몬 반품취소, 스토아 반품완료 확인 중
                                       ,
                                        '3121210930' -- 2/10
                                       ,
                                        '30000129478895' -- 2/22 부분 회수확
                                        )
  ...
  SELECT CASE
    ...
    FROM (SELECT (SELECT SUM(POC.SHPFEE_COST)
                 ...
                 (SELECT SUM(POC.SHPFEE_COST)
                    FROM TORDERSHIPCOST POC
                   WHERE POC.ORDER_NO = PO.ORDER_NO
                     AND POC.TYPE IN ('20', '31', '41')) AS M_SHPFEE_COST,
                 (SELECT SUM(ORC.QUEST_AMT)
                    FROM TORDERRECEIPTS ORC
                   WHERE ORC.ORDER_NO = PO.ORDER_NO
  ...
  */
  ...
  SELECT CASE
    ...
    FROM (SELECT CD.ORDER_NO,
           ...
           WHERE CD.CANCEL_DATE >= TO_DATE(V_FROMDATE, 'YYYYMMDDHH24MISS')
             ...
             AND TM.MOB_POC_GB = '20'
             AND CD.GOODS_GB = '10'
             AND CD.ORDER_NO NOT IN ('20220522472419' -- 5/26
                                     )
             AND NOT EXISTS
           (SELECT 1
                    FROM TPAORDERM PM
    ...
    FROM (SELECT CL.ORDER_NO,
           ...
           (SELECT 1
                   ...
                   WHERE PM.CREATE_YN = '1'
                     ...
                     AND PM.ORDER_W_SEQ = CL.ORDER_W_SEQ
                     AND TO_NUMBER(PM.PA_PROC_QTY) = CL.CLAIM_QTY)
             AND CL.ORDER_NO NOT IN ('20220627183638' -- 7/6
                                    ,
                                     '20220623687921' -- 7/6
                                    ,
                                     '20220625270718' -- 7/6
                                    ...
                                    ,
                                     '20220917939642' -- 9/19
                                    ,
                                     '20220705460089' -- 11/17
                                    ,
                                     '20221111116954' -- 11/23
                                    ,'20220921411969' -- 1/11
                                    ,'20220628387088' -- 1/31
                                     ));
  ...
  SELECT CASE
    ...
    FROM (SELECT CL.ORDER_NO,
           ...
           WHERE CL.MEDIA_GB = '03'
             ...
             AND CL.CLAIM_DATE >= TO_DATE(V_FROMDATE, 'YYYYMMDDHH24MISS')
             AND CL.CLAIM_GB = '40'
             AND CL.ORDER_NO NOT IN ('20211227400498' -- 1/6
                                    ,
                                     '20211230972520' -- 1/7
                                     )
             AND NOT EXISTS
  ...
  SELECT CASE
    ...
    FROM (SELECT PM.PA_ORDER_NO,
           ...
           WHERE PM.PA_ORDER_NO = PT.ORD_NO
             ...
             AND PM.CREATE_YN = '1'
             AND PM.PA_ORDER_NO NOT IN ('20220305386753846' --3/25 ( 취소승인처리 미노출 옵션코드보정 )
                                        )
           ...
           WHERE OL.INSERT_DATE BETWEEN TO_DATE(V_TODATE, 'yyyymmdd') - 2 AND
             ...
             AND NOT EXISTS (SELECT 1
                   ...
                   WHERE PM.PA_GROUP_CODE = '01'
                     ...
                     AND PM.PA_ORDER_NO = OL.ORD_NO
                     AND ROWNUM = 1)
             AND OL.ORD_NO NOT IN ('20220424422815052',
                                   '20220522444030377' -- 5/23
                                  ,
                                   '20220523444792545' -- 5/23
                                  ,
                                   '20220710476906308' -- 7/11
                                  ,
                                   '20220712479918757' -- 7/13
                                  ,
                                   '20221031549011063' -- 10/31
                                  ,
                                   '20221106554862318' -- 11/7
                                  ,
                                   '20221107555920611',
                                   '20221107556059948',
                                   '20221107556063051',
                                   '20221107556091739' -- 11/8
                                  ,'20221122566991386' -- 11/22
                                  ,'20221207576130882' -- 12/8
                                  ,'20221227588193519' -- 12/28
                                  ,'20221229589254811' -- 12/30
                                  ,'20230109594931551' -- 1/9
                                   )
  ...
  SELECT CASE
    ...
    FROM (SELECT OD.ORDER_NO,
            ...
            FROM TPAORDERM PM,
                 ...
                 TORDERDT OD,
                 TGOODSDT GT,
                 (SELECT MAX(OD2.ORDER_NO) AS MAX_ORDER_NO,
                         MIN(OD2.ORDER_NO) AS MIN_ORDER_NO
                    FROM TORDERDT OD2
                   WHERE OD2.ORDER_DATE >= SYSDATE - 30
                     AND OD2.MEDIA_CODE = 'EX01') CC
           WHERE PM.PA_ORDER_GB = OL.PA_ORDER_GB
             AND PM.PA_ORDER_NO = OL.ORD_NO
             AND PM.PA_ORDER_SEQ = OL.ORD_PRD_SEQ
             AND PM.PA_SHIP_NO = OL.DLV_NO
             ...
             AND SUBSTR2(OL.SLCT_PRD_OPT_NM, 5) <>
                 TRIM(GT.GOODSDT_INFO) || '-' || OD.ORDER_QTY || '개'
             AND OD.ORDER_NO BETWEEN CC.MIN_ORDER_NO AND CC.MAX_ORDER_NO
  ...
  SELECT CASE
    ...
    FROM (SELECT /*+FULL(PM)*/
           ...
           OD.ORDER_QTY,
           OD.ORDER_DATE
            FROM TORDERDT OD, TPAORDERM PM, TPA11STORDERLIST OL
  ...
  SELECT CASE
    ...
    INTO IS_EXISTED
    FROM (SELECT /*+FULL(PM)*/
           OD.ORDER_NO, OD.ORDER_G_SEQ, OD.REMARK3_N, OL.LST_SELLER_DSC_PRC
            FROM TORDERDT OD, TPAORDERM PM, TPA11STORDERLIST OL
           WHERE OD.ORDER_NO = PM.ORDER_NO
             AND OD.ORDER_G_SEQ = PM.ORDER_G_SEQ
             AND OD.ORDER_D_SEQ = PM.ORDER_D_SEQ
  ...
  SELECT CASE
    ...
    FROM (SELECT TR.CUST_NO,
           ...
           WHERE CUST_NO IN
                 (SELECT DISTINCT CUST_NO
                    FROM TORDERDT OD
                   WHERE OD.MEDIA_CODE LIKE 'EX%'
                     AND OD.SYSLAST > 0
                     AND OD.DO_FLAG > 25
                     AND OD.ORDER_DATE BETWEEN SYSDATE - 5 AND SYSDATE))
   WHERE ((HP IS NULL AND TEL = '000000000000') OR
     ...
     AND CUST_NO NOT IN ('202112132848' -- 12/3
                         );
  ...
  SELECT CASE
         ...
         END DATA_CHECK
    INTO IS_EXISTED
    FROM (SELECT OL.TOTAL_PRODUCT_AMT AS PANAVER_SALE_PRICE,
  ...
  SELECT CASE
         ...
         END DATA_CHECK
    INTO IS_EXISTED
    FROM (SELECT /*+full(pm)*/OD.ORDER_NO,
                 OL.ORDER_ID,
                 OL.PRODUCT_ORDER_ID,
                 OL.OPTION_CODE,
                 OL.PRODUCT_OPTION,
           ...
           WHERE PM.PA_ORDER_GB = OL.PA_ORDER_GB
             ...
             AND OD.GOODS_CODE = GT.GOODS_CODE
             AND OD.GOODSDT_CODE = GT.GOODSDT_CODE
             AND SUBSTR2(OL.PRODUCT_OPTION, 5) || '-' ||
                 TO_CHAR(OL.QUANTITY) || '개' <>
                 TRIM(GT.GOODSDT_INFO) || '-' || OD.ORDER_QTY || '개'
             AND OD.GOODSDT_CODE <> '001'
             AND GT.MODIFY_DATE < OD.ORDER_DATE
             AND OD.ORDER_DATE >= SYSDATE - 5
             AND PM.PA_ORDER_NO NOT IN ('2022082997710061' -- 8/29
                                       ,
                                        '2022090220998701' -- 9/2
                                       ,
                                        '2022091291061901' -- 9/13
                                       ,
                                        '2022100655433281' -- 10/7
                                       ,
                                        '2022102741265421'
                                       ,'2022102743100341' -- 10/27
                                       ,'2022121062379581' --12/12
                                       ,'2023011017896681' -- 1/16
                                        )
           ORDER BY OD.ORDER_NO DESC);
  ...
  SELECT CASE
    ...
    FROM (SELECT O.ORDER_ID, O.PRODUCT_ORDER_ID
           ...
           WHERE T.HOLDBACK_STATUS = '10'
             ...
             AND NOT EXISTS (SELECT 1
                    FROM TPAORDERM P
                   WHERE P.PA_CLAIM_NO = O.CLAIM_ID
                     AND P.PA_ORDER_GB = CASE
                           WHEN T.CLAIM_STATUS LIKE '1%' THEN
                            '14'
                           WHEN T.CLAIM_STATUS LIKE '2%' THEN
                            '25'
                         END)
             AND NOT EXISTS
           (SELECT 1
  ...
  */
  ...
  SELECT CASE
    ...
    FROM (SELECT PA_ORDER_GB, PA_SHIP_NO
          ...
          --반품처리완료.
           WHERE PA_SHIP_NO NOT IN ('G1182236034' -- 12/6 주문생성전 취소
                                   ,'G1185623358' -- 12/8 주문생성전 취소
                                   ,'G1185544458' -- 12/9 주문생성전 취소
                                   ,'G1186995290' -- 12/13 주문생성전 취소
                                   ,'338874881'
                                   ,'G1193189634' -- 12/21 주문생성전 취소
                                   ,'G1193437526' -- 12/22 주문생성전 취소
                                   ,'G1200518586'
                                   ,'G1201068090' -- 1/4 주문생성전 취소
                                   ,'G1210131866' -- 1/17 주문생성전 취소
                                    )
  ...
  SELECT CASE
         ...
         END DATA_CHECK
    INTO IS_EXISTED
    FROM (SELECT DISTINCT PM.PA_ORDER_NO, PM.ORDER_NO
            FROM TPOST            P,
                 TPAHALFGOODS     HG,
                 TPAORDERM        PM,
                 TORDERDT         TD,
            ...
            WHERE PM.PA_CODE IN ('C1', 'C2')
             ...
             AND HO.ORD_NO NOT IN ('220919347803'));
  IF (IS_EXISTED > 0) THEN
    INSERT_ERROR_LOG();
  END IF;
  ...
  SELECT CASE
    ...
    FROM ( SELECT HE.ORD_NO AS PA_ORDER_NO
            ...
            WHERE HE.INSERT_DATE > SYSDATE - 3
              AND HE.ORD_NO NOT IN ('230117293209','221227922343','230104337193','221231127045'));
  IF (IS_EXISTED > 0) THEN
    INSERT_ERROR_LOG();
  END IF;  
    ...
    FROM (SELECT '01' AS PA_GROUP_CODE, OL.ORD_NO
          ...
          SELECT '04' AS PA_GROUP_CODE, OL.ORDER_ID
           ...
           WHERE OL.PA_ORDER_GB = '10'
             AND OL.PRODUCT_ORDER_STATUS != '29'
             AND OL.INSERT_DATE >= TO_DATE(V_FROMDATE, 'YYYYMMDDHH24MISS')
             AND NOT EXISTS
           (SELECT 1
                    FROM TPAORDERM PM
  ...
  V_FLAG      := 'PI012';
  V_ERROR_MSG := '[PI10] I/F 교환회수 취소, 교환배송 취소 검증 ( TPAORDERM <-> 제휴테이블 )';
  IS_EXISTED  := 0;
  ...
  SELECT CASE
    ...
    FROM (SELECT PM.PA_CODE, TD.ORDER_NO, TD.ORDER_DATE
           ...
           WHERE 1 = 1
             ...
             AND PM.ORDER_NO NOT IN ('20220628409047' -- 7/1
                                    ,
                                     '20220701981103' -- 7/4
                                    ,
                                     '20220701944203' -- 7/4
